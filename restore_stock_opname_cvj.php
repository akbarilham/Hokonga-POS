...
$mmenu = "restore";
$smenu = "restore_stock_opname_cvj";
...
?>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="restore_stock_opname_cvj.php">
								...
								<input type='hidden' name='mb_type' value='<?=$mb_type?>'>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="data_name" type="text" value="summary_cvj_2015" />
                                        </div>
                                    </div>
...
<?
	...
	if($job_slct == "4") {
		...
		echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_cvj.php'>");
		exit;
		...
		$queryC2 = "SELECT count(pnum) FROM summary_cvj_2015";
		$resultC2 = mysql_query($queryC2);
			if (!$resultC2) { error("QUERY_ERROR"); exit; }
		$total_recordC2 = @mysql_result($resultC2,0,0);

		$queryD2 = "SELECT pnum,org_pcode,stocks FROM summary_cvj_2015 ORDER BY pnum ASC";
		...
		$queryA1 = "SELECT count(num) FROM table_store_name_cvj";
		$resultA1 = mysql_query($queryA1);
			if (!$resultA1) { error("QUERY_ERROR"); exit; }
		$count1 = @mysql_result($resultA1,0,0);
  		
  		for($o1=0; $o1<78; $o1++) {
	  		$num = $o1 + 1; 
	  		$queryA2 = "SELECT shop_code,shop_name FROM table_store_name_cvj where num = '$num' order by shop_code ASC";
			$resultA2 = mysql_query($queryA2);
				if (!$resultA2) { error("QUERY_ERROR"); exit; }
			$shop = @mysql_result($resultA2,0,0);
			$nameshop = @mysql_result($resultA2,0,1);

			$queryC2 = "SELECT count(shop_code),shop_code FROM summary_cvj_2015 where shop_code = '$shop' group by shop_code asc";
			$resultC2 = mysql_query($queryC2);
				if (!$resultC2) { error("QUERY_ERROR"); exit; }
			$count2 = @mysql_result($resultC2,0,0);
			$shop2 = @mysql_result($resultC2,0,1);

			$queryD2 = "SELECT pnum,org_pcode,shop_code,stocks,actual_balance FROM summary_cvj_2015 WHERE shop_code = '$shop' ORDER BY shop_code ASC";
			$resultD2 = mysql_query($queryD2);
				if (!$resultD2) { error("QUERY_ERROR"); exit; }

			echo 'Item ===['.$count2.']=== Shop Code : ['.$shop2.']|['.$nameshop.'] </br>';

				for($ra=0; $ra < $count2; $ra++){		
					
					$R_pnum = mysql_result($resultD2,$ra,0);
					$R_pcode = mysql_result($resultD2,$ra,1);
					$R_shopcode = mysql_result($resultD2,$ra,2);
					$R_stores = mysql_result($resultD2,$ra,3);
					$R_total_qty = mysql_result($resultD2,$ra,6);
					...
					$query_sh = "SELECT shop_code,branch_code,associate,group_code FROM client_shop WHERE shop_code= '$shop' and branch_code='CORP_03'";
					...
					$x = 8; // Ganti bulan disini
					$y = 6;	// jumlah kolom per toko
					$z = 2; // 
					$cross = $x * $y;
					$a = $cross - $z;
					#if($sp_opt1[$a] > 0) {
						...
						// Data Entry - Stock-out
							$query_SH1 = "INSERT INTO shop_product_list_qty (uid,org_uid,flag,branch_code,supp_code,group_code,shop_code,gudang_code,
								...
								'$sp_opt1[$a]','0','0','$post_dates','$post_dates','0','1')";
							...
							$query_dir2 = "INSERT INTO shop_stock_list (uid,branch_code,brand_code,supp_code,group_code,shop_code,gudang_code,
									...
									'$li_catg_code','$li_gcode','$li_pcode','$R_pcode','$li_org_barcode','$sp_opt1[$a]','$sp_opt1[$a]','$li_gname','$li_pname')";
							...
							$query_SH2 = "INSERT INTO shop_product_list_shop (uid,branch_code,catg_code,brand_code,group_code,shop_code,
								...
								'$li_gcode','$li_pcode','$R_pcode','$li_org_barcode','$li_gname','$li_pname','$sp_opt1[$a]','$sp_opt1[$a]','$post_dates','1')";
							$result_SH2 = mysql_query($query_SH2);
							if (!$result_SH2) { error("QUERY_ERROR"); exit; }
							
							
							
							echo (" ---- $num ... [$sh_branch_code][$sh_shop_code] $sh_group_code ( $sh_associate ) - $sp_opt1[$a]</br>");
							
							# echo '<br/>'.$R_shopcode.' array agustus actual_balance : '.$sp_opt1[$a];
