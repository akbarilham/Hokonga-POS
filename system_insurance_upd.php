...
<?
$query = "SELECT uid,branch_code,agent_code,agent_name,branch_name,contact_name,manager,email,homepage,phone1,phone2,phone_fax,phone_cell,
		addr1,addr2,zipcode,userlevel,signdate,memo	FROM code_insurance WHERE uid = '$uid'";
$result = mysql_query($query);
	if (!$result) {   error("QUERY_ERROR");   exit; }
$user_uid = @mysql_result($result,0,0);
$branch_code = @mysql_result($result,0,1);
$agent_code = @mysql_result($result,0,2);
$agent_name = @mysql_result($result,0,3);
$branch_name = @mysql_result($result,0,4);
$contact_name = @mysql_result($result,0,5);
$manager = @mysql_result($result,0,6);
$email = @mysql_result($result,0,7);
$homepage = @mysql_result($result,0,8);
$phone1 = @mysql_result($result,0,9);
$phone2 = @mysql_result($result,0,10);
$phone_fax = @mysql_result($result,0,11);
$phone_cell = @mysql_result($result,0,12);
$addr1 = @mysql_result($result,0,13);
$addr2 = @mysql_result($result,0,14);
$zipcode = @mysql_result($result,0,15);
$userlevel = @mysql_result($result,0,16);
$signdates = @mysql_result($result,0,17);
$memo = @mysql_result($result,0,18);
...
?>
        ...
        <header class="panel-heading">
            ...
            <span class="tools pull-right">
				<a href="system_insurance_del.php?uid=<?=$user_uid?>" class="fa fa-trash-o"></a>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="system_insurance_upd.php">
								...
								<input type='hidden' name='key' value='<?=$key?>'>
									...
									<div class="form-group ">
                                        <label for="cname" class="control-label col-sm-3"><?=$txt_sys_insur_05?></label>
                                        <div class="col-sm-3">
                                            <input readonly class="form-control" id="cname" name="dis_agent_code" value="<?=$agent_code?>" type="text" />
                                        </div>
										<div class="col-sm-3"><?=$txt_comm_frm23?></div>
										<div class="col-sm-3">
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="agent_name" value="<?=$agent_name?>" type="text" required />
                                        </div>
                                    </div>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="contact_name" value="<?=$contact_name?>" type="text" />
                                        </div>
                                    </div>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="agent_manager" value="<?=$manager?>" type="text" />
                                        </div>
                                    </div>
...
<?
  ...
  $query  = "UPDATE code_insurance SET userlevel = '$userlevel', agent_name = '$agent_name', 
              branch_name = '$branch_name', contact_name = '$contact_name', manager = '$agent_manager', 
			  email = '$email', homepage = '$homepage', zipcode = '$zipcode', 
              addr1 = '$addr1', addr2 = '$addr2', phone1 = '$phone1', phone2 = '$phone2', phone_fax = '$phone_fax', 
              phone_cell = '$phone_cell', memo = '$memo', branch_code = '$user_branch_code' WHERE uid = '$user_uid'";
  $result = mysql_query($query);
  if(!$result) { error("QUERY_ERROR"); exit; }
