...
$link_list = "$home/inventory_opname.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
$link_list_action = "$home/inventory_opname.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=upd&view=qty";

$link_post = "$home/inventory_opname.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
$link_post_catg = "$home/inventory_opname.php?mode=post_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
...
?>
        ...
        <header class="panel-heading">
            <?=$hsm_name_02_022?> &nbsp;&nbsp; 
			<a href='inventory_opnameD.php'><i class='fa fa-list-ul'></i></a>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='inventory_opname.php'>
			<div class='col-sm-2'>
...
$query_tsm2 = "SELECT sum(stock_loss) FROM shop_product_list_qty 
			WHERE (flag = 'out' AND shop_code = '$login_shop') 
...
for($i = $first; $i <= $last; $i++) {
   ...
   $prd_qty = mysql_result($result,$i,8);
	...
	$query_sub2 = "SELECT sum(stock_loss) FROM shop_product_list_qty 
			WHERE (gcode = '$prd_gcode' AND flag = 'out' AND shop_code = '$login_shop') 
			OR (gcode = '$prd_gcode' AND flag = 'out' AND shop_code2 = '$login_shop')";
  ...
  if($mode2 == "dtl" AND $gcode == $prd_gcode) {
    ...
    $query_H = "SELECT uid,gate,catg_code,gcode,pcode,pname,
      ...
      product_option4,product_option5,price_orgin,org_pcode,org_barcode,gudang_code FROM shop_product_list WHERE gcode = '$prd_gcode' 
    ...
    for($h = 0; $h < $total_HC; $h++) {
	  ...
	  $H_org_pcode = mysql_result($result_H,$h,17);
	  $H_org_barcode = mysql_result($result_H,$h,18);
	  $H_gudang_code = mysql_result($result_H,$h,19);
		...
		$query_subx2 = "SELECT sum(stock_loss) FROM shop_product_list_qty 
			WHERE (pcode = '$H_prd_code' AND flag = 'out' AND shop_code = '$login_shop') 
			OR (pcode = '$H_prd_code' AND flag = 'out' AND shop_code2 = '$login_shop')";
    ...
    $query_qs1 = "SELECT count(uid) FROM shop_product_list_qty 
			WHERE (pcode = '$H_prd_code' AND shop_code = '$login_shop') 
			OR (pcode = '$H_prd_code' AND shop_code2 = '$login_shop')";
    ...
    <tr>
      ...
      <td bgcolor='$highlight_color_L'>$H_org_pcode</td>
      ...
      <td bgcolor='$highlight_color_L'>
		...
		if($act_mode == "add" AND $uid == $H_prd_uid) {
			...
			if($pos_type == "3") { // Consignment Store
				$check_vitual_txt = $txt_invn_stockin_66;
			} else {
				$check_vitual_txt = $txt_invn_stockin_63;
			}
		
		  echo ("
          <tr>
            <form name='qs_signform1' method='post' action='inventory_opname.php'>
			...
			<input type='hidden' name='li_gudang_code' value='$H_gudang_code'>
			<input type='hidden' name='org_sumx_qty_now' value='$sumx_qty_now2'>
			
			

            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td align=right>$check_vitual_txt</td>
			<td align=right><input type='text' class='form-control' name='vstock_check_now' style='width: 60px; height: 23px; text-align: right; font-size: 1.0em' required></td>
			<td align=right></td>
			<td align=right></td>
			<td align=right></td>
      ...
      if($view == "qty" AND $uid == "$H_prd_uid") {
        ...
        $query_qs2 = "SELECT uid,stock,date,flag,pay_num,gudang_code,supp_code,shop_code,org_uid,stock_check,stock_loss,stock_org,flag,virtual,
					branch_code,check_date FROM shop_product_list_qty WHERE (pcode = '$H_prd_code' AND shop_code = '$login_shop') 
					OR (pcode = '$H_prd_code' AND shop_code2 = '$login_shop') ORDER BY check_date DESC, date DESC";
        ...
        for($qs = 0; $qs < $total_qs; $qs++) {
		  ...
		  $qs_branch_code = mysql_result($result_qs2,$qs,14);
		  $qs_dateB = mysql_result($result_qs2,$qs,15);
		  ...
		  $query_qs7 = "SELECT branch_name FROM client_branch WHERE branch_code = '$qs_branch_code'";
          $result_qs7 = mysql_query($query_qs7,$dbconn);
          if (!$result_qs7) { error("QUERY_ERROR"); exit; }   
          
          $qs_branch_name = @mysql_result($result_qs7,0,0);
          ...
          <tr>
            ...
            <td>$qs_flag_txt</td>
            <td align=right>
				<a href='#' class='tooltips' title=\"\" data-toggle='tooltip' data-placement='top' data-original-title='$qs_branch_name'>[$qs_branch_code]</a> 
			...
			<td align=right>$qs_stock_loss_txt</td>
...
<?
  ...
  $post_dates = "$post_date1"."$post_date2";
	...
	echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_opname.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$li_uid&page=$page&view=qty&pos_type=$pos_type&view=qty&mode2=$mode2&gcode=$gcode'>");
	exit;
