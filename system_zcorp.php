...
$link_list = "$home/system_zcorp.php";
$link_post = "$home/system_zcorp_post.php";
$link_upd = "$home/system_zcorp_upd.php";
$link_del = "$home/system_zcorp_del.php";
?>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,branch_code,branch_name,branch_name2,ceo_name,userlevel,signdate
    FROM client_branch ORDER BY userlevel DESC, branch_code ASC";
} else {
   $query = "SELECT uid,branch_code,branch_name,branch_name2,ceo_name,userlevel,signdate
    FROM client_branch WHERE $keyfield LIKE '%$key%' ORDER BY userlevel DESC, branch_code ASC";
}
...
for($i = $first; $i <= $last; $i++) {
   ...
   $branch_name2 = mysql_result($result,$i,3);
		$branch_name2 = stripslashes($branch_name2);
   $ceo_name = mysql_result($result,$i,4);
  ...
  echo ("<tr>");
  echo("<td><a href='$link_upd?uid=$uid'>{$branch_code}</a></td>");
  echo("<td><a href='$link_upd?uid=$uid'>{$branch_name}</a></td>");
  echo("<td><a href='$link_upd?uid=$uid'>{$branch_name2}</a></td>");
  ...
  if($userlevel == "0") {
	  ...
	  $level_name = "<font color=black>Corporate</font>";
	} else if($userlevel == "5") {
	  $level_name = "Head Quarter";
  } else {
	  $level_name = "-!-";
  }
