...
<section id="container" class="">
	...
	<section id="main-content">
		...
		<?
		echo ("
		<form name='signform1' method='post' action='system_wloc_catg_post.php'>
...
<?
  	...
  	if($b_code == "add") {
	    ...
	    $query_L1  = "INSERT INTO wms_catgbig (lcode, lname, branch_code) VALUES ('$add_code','$b_name','$login_branch')";
	    $result_L1 = mysql_query($query_L1,$dbconn);
	    if(!$result_L1) { error("QUERY_ERROR"); exit; }

      echo("<meta http-equiv='Refresh' content='0; URL=$home/system_wloc_catg.php?gate=$login_gate'>");
      exit;
...
<section id="container" class="">
	...
	<section id="main-content">
		...
		<form name='signform2' method='post' action='system_wloc_catg_post.php'>
		<input type='hidden' name='step_next' value='permit_okay'>
		<input type='hidden' name='gate' value='$login_gate'>
		...
		$mid_query = "SELECT * FROM wms_catgmid WHERE lcode = '$b_code' ORDER BY mcode";
...
<?
	...
	$b_code = substr($addcode, 0,2); // Main Category Code
	$m_name = addslashes($m_name);

	$query_M1  = "INSERT INTO wms_catgmid (lcode, mcode, mname, branch_code) VALUES ('$b_code','$addcode','$m_name', '$login_branch')";
