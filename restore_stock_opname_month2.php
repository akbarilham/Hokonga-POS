...
$link_list = "$home/restore_stock_opname_month2.php";
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(org_pcode) FROM table_stock_201507_cvj2";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(org_pcode) FROM table_stock_201507_cvj2 WHERE $keyfield LIKE '%$key%'";  
}
...
$query_ini = "SELECT count(org_pcode) FROM table_stock_201507_cvj2";
...
?>
        ...
        <div class="panel-body">
			...
			echo ("
			<form name='search' method='post' action='restore_stock_opname_month2.php'>
			<div class='col-sm-3'>
        ...
        <?
		if(!eregi("[^[:space:]]+",$key)) {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_cvj2 ORDER BY $sorting_key $sort_now";
		} else {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_cvj2 WHERE $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
		}
