...
$link_list = "$home/sales_order3.php?sorting_key=$sorting_key&key_shop=$key_shop";
$link_upd = "$home/sales_order3.php?sorting_key=$sorting_key&key_shop=$key_shop&keyfield=$keyfield&key=$key&page=$page";
?>
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='sales_order3.php'>
			<div class='col-sm-2'>
...
$time_limit = 60*60*24*$notify_new_article; 
  ...
  if(!eregi("[^[:space:]]+",$key)) {
		$query = "SELECT uid,shop_code,catg_code,pcode,pname,price_orgin,price_sale,price_market,
				stock_org,stock_now,stock_sell,sold_out,product_color,product_size,
				product_option1,product_option2,product_option3,product_option4,product_option5,org_pcode,org_barcode,gname FROM shop_product_list 
	...
	if($keyfield == "all") {
		$query = "SELECT uid,shop_code,catg_code,pcode,pname,price_orgin,price_sale,price_market,
				stock_org,stock_now,stock_sell,sold_out,product_color,product_size,
				product_option1,product_option2,product_option3,product_option4,product_option5,org_pcode,org_barcode,gname FROM shop_product_list 
				WHERE org_pcode LIKE '%$key%' OR gname LIKE '%$key%' OR pname LIKE '%$key%' ORDER BY $sorting_key $sort_now";
	} else {
		$query = "SELECT uid,shop_code,catg_code,pcode,pname,price_orgin,price_sale,price_market,
				stock_org,stock_now,stock_sell,sold_out,product_color,product_size,
				product_option1,product_option2,product_option3,product_option4,product_option5,org_pcode,org_barcode,gname FROM shop_product_list 
...
for($i = $first; $i <= $last; $i++) {
   ...
   $prd_pname = mysql_result($result,$i,4);
		$prd_pname = stripslashes($prd_pname);
   $prd_price_orgin = mysql_result($result,$i,5);
      $prd_price_orgin_K = number_format(floatval($prd_price_orgin));
  ...
  echo("
		<td bgcolor='$highlight_color'><a href='#' class='tooltips' title=\"\" data-toggle='tooltip' data-placement='top' data-original-title='$prd_catg_full'>$prd_catg</a></td>
		<td bgcolor='$highlight_color'><a href='$link_upd$page&mode=search&uid=$prd_uid' data-placement='top' 
		data-toggle='tooltip' class='tooltips' data-original-title='[$H_gcode] $prd_gname'>$prd_pname {$H_p_opt1_txt}{$H_p_opt2_txt}{$H_p_opt3_txt}{$H_p_opt4_txt}{$H_p_opt5_txt}</a></td>");
  ...
  // $link_upd$page get value from $page for select item 
	// Cart
	...
	echo ("
	<form name='p_cart_dtl' method='post' action='sales_order_cart2.php'>
	...
	<td bgcolor='$highlight_color'><input type='text' name='cart_qty' value='0' maxlength=6 value='10' 
				style='width:60px; height: 20px; padding-right: 5px; text-align: right'></td>
...
?>
			...
			<div class="col-sm-2">
				...
				echo ("
				<form name='product_update_all' method='post' action='sales_order3.php'>
			...
			<div class="col-sm-2">
				...
				<div class="form-actions">
					<a href="sales_order2.php"><button class="btn btn-danger"><i class="fa fa-shopping-cart"></i> <?=$txt_sales_sales_05?></button></a>
				</div>
      ...
      // 합계
      echo ("
      <form name='order_check' method='post' action='sales_order3.php'>
	  ...
	  if($smode == "order_confirm") {
		...
		echo ("
		<form name='order_check' class='cmxform form-horizontal adminex-form' method='post' action='sales_order3.php'>
...
<?
  ...
  if($add_mode == "update_all") { // 모든 상품 업데이트
      ...
      // 리스트로 돌아가기
      echo("<meta http-equiv='Refresh' content='10; URL=$home/sales_order3.php?sorting_key=$sorting_key&key_shop=$key_shop&keyfield=$keyfield&key=$key&page=$page'>");
