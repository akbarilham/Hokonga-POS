...
$mmenu = "restore";
$smenu = "restore_stock_opname_monthlnl";
...
$link_list = "$home/restore_stock_opname_monthlnl.php";
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(org_pcode) FROM table_stock_201507_lnl";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(org_pcode) FROM table_stock_201507_lnl WHERE $keyfield LIKE '%$key%'";  
}
...
$query_ini = "SELECT count(org_pcode) FROM table_stock_201507_lnl";
...
?>
        ...
        <div class="panel-body">
			...
			echo ("
			<form name='search' method='post' action='restore_stock_opname_monthlnl.php'>
			<div class='col-sm-3'>
        ...
        <?
		if(!eregi("[^[:space:]]+",$key)) {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_lnl ORDER BY $sorting_key $sort_now";
		} else {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_lnl WHERE $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
		}
		...
		<div class='col-sm-9'>
				...
				<tr>
				<?
					$store_branch = "CORP_04";
					$store_branch_name = "L&L";
					$total_ini_store = 129;
					...
					for($num=1; $num<=$total_ini_store; $num++) {
						...
						$query_str = "SELECT shop_code FROM table_store_name_lnl WHERE num = '$num2'";
						...
						$query_cd = "SELECT shop_code,shop_name FROM client_shop WHERE shop_code = '$shop_code'";
						$result_cd = mysql_query($query_cd);
							if (!$result_cd) { error("QUERY_ERROR"); exit; }
						$new_shop_code = @mysql_result($result_cd,0,0);
						$new_shop_name = @mysql_result($result_cd,0,1);
...
<?
	...
	echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_monthlnl.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
	...
	echo("<meta http-equiv='Refresh' content='10; URL=$home/restore_stock_opname_monthlnl.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
