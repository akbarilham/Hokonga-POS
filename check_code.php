...
$uid_per_page = 20; // uidber of article lines per page
...
if(!$total_record) {
   ...
   $first = $uid_per_page*($page-1);
   $last = $uid_per_page*$page;
...
$total_page = ceil($total_record/$uid_per_page);
?>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,shop_name,shop_code,org_shop_code
    FROM client_shop ORDER BY shop_code ASC";
} else {
   $query = "SELECT uid,shop_name,shop_code,org_shop_code
    FROM client_shop WHERE $keyfield LIKE '%$key%' ORDER BY shop_code ASC";
}

$result = mysql_query($query);
if (!$result) {   error("QUERY_ERROR");   exit; }

$article_uid = $total_record - $uid_per_page*($page-1);
