...
$link_list = "$home/system_bank.php";
$link_post = "$home/system_bank_post.php?keyfield=$keyfield&key=$key";
$link_upd = "$home/system_bank_upd.php";
$link_del = "$home/system_bank_del.php";
?>
...
// Total Count ---------------------------------------------------- //
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(uid) FROM code_bank";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(uid) FROM code_bank WHERE $keyfield LIKE '%$key%'";  
}
...
?>
        ...
        <tr>
            ...
            <th><?=$txt_sys_bank_05?></th>
			<th><?=$txt_sys_bank_06?></th>
			<th><?=$txt_sys_bank_07?></th>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,branch_code,bank_code,bank_name,account_code,account_name,manager,userlevel,signdate,currency
    FROM code_bank ORDER BY bank_code ASC";
} else {
   $query = "SELECT uid,branch_code,bank_code,bank_name,account_code,account_name,manager,userlevel,signdate,currency
    FROM code_bank WHERE $keyfield LIKE '%$key%' ORDER BY bank_code ASC";
}
...
for($i = $first; $i <= $last; $i++) {
   ...
   $bank_code = mysql_result($result,$i,2);
   $bank_name = mysql_result($result,$i,3);
   $account_code = mysql_result($result,$i,4);
   $account_name = mysql_result($result,$i,5);
   $bank_manager = mysql_result($result,$i,6);
   $userlevel = mysql_result($result,$i,7);
   $signdate = mysql_result($result,$i,8);
   $bank_curr = mysql_result($result,$i,9);
    if($lang == "ko") {
  ...
  echo("<td><a href='$link_upd?uid=$uid&keyfield=$keyfield&key=$key'>{$bank_code}</a></td>");
  echo("<td><a href='$link_upd?uid=$uid&keyfield=$keyfield&key=$key'>{$bank_name} ($bank_curr)</a></td>");
