...
function onEnterC(e){
  ...
  if(key==13){
    ...
    showTotal();
    getClear();
  $("#test").load("<?=$home?>/boomsale_preview.php");
   $('#form')[0].reset();
...
function showCell(){
  ...
  valItem=rObj.value.trim();
    ...
    doRequested('viewResult'+index,'boomsale_insert.php?val='+valBarcode+'&index='+newIndex+'&qtys='+valQty+'&uid='+valUid+'&item='+valItem,false);
  }
}
			...
			<script>
					...
					$(document).ready(function(){
						$("a#link").click(function(){
							...
							var new_url = ext+'/boomsale.php?user='+ext2+'&va='+va;
              ...
              <section class="panel">
                <form name='clear' id='clear' method='post' action="<?=$_SERVER['PHP_SELF'];?>">
                  ...
                  if($delete==$clr) { 
                    ...
                    echo( "<meta http-equiv='Refresh' content='0; URL=$home/boomsale_add.php'>"); exit; } 
                    ?>
                </form>
