...
$mmenu = "system";
$smenu = "system_category";

$link_list = "$home/system_category.php";
$link_post = "$home/system_category_post.php";
$link_upd = "$home/system_category_upd.php";
$link_del = "$home/system_category_del.php";
?>
...
echo ("
		...
		$big_query = "SELECT * FROM shop_catgbig WHERE lang = '$lang' ORDER BY lcode";
		...
		while($big_row = mysql_fetch_object($big_result)) {
		  	...
		  	$big_imgtype = $big_row->image_type;
		  	$big_onoff = $big_row->onoff;
		  	$group_code = $big_row->group_code;
		  	$prg_type = $big_row->prg_type;
		  	
		  	$j2 = $j + 1;
		  	
		  	// 활성화
		  	if($big_onoff == "1") {
		  	  $big_onoff_chk = "checked";
		  	} else {
		  	  $big_onoff_chk = "";
		  	}
		  	
		  	// 중분류
		  	$mid_query = "SELECT * FROM shop_catgmid WHERE lang = '$lang' AND lcode = '$big_code' ORDER BY mcode";
			...
			// 대분류 수정 링크
			echo ("
			<form name='formB' method='post' action='$home/process_catg_updB.php'>
			...
			<td rowspan=$mid_rows>
				...
				<tr>
					...
					<td>
						...
						if($j2 == $big_rows) {
							echo("<a href='$home/process_catg_del.php?lang=$lang&mode=big&bcode=$big_code&bname=$big_name&gate=$gate'>$txt_comm_frm13</a>");
						} else {
			...
			");
		  	...
		  	while($mid_row = mysql_fetch_object($mid_result)) {
		    	...
		    	if($i == 0) {
		      	echo ("
				<form name='formM' method='post' action='$home/process_catg_updM.php'>
				<input type='hidden' name='lang' value='$lang'>
				<input type='hidden' name='gate' value='$login_gate'>
					...
					<tr>
						<td height=16>($m_code)
							...
							if($ss_mcode != $m_code) { echo ("
								<a href='system_category.php?lang=$lang&ss_mode=add&ss_mcode=$m_code'><i class='fa fa-pencil'></i> Add</a>");
							} echo ("
						</td>
						...
						<td align=right>
							...
							if($i2 == $mid_rows) {
								echo("<a href='$home/process_catg_del.php?lang=$lang&mode=mid&mcode=$m_code&mname=$m_name&gate=$gate'>$txt_comm_frm13</a>");
							} else {
								echo("<font color=#AAAAAA>$txt_comm_frm13</font>");
							}
				...
				<td colspan=2>
					...
					// 소분류
					$sml_query = "SELECT * FROM shop_catgsml 
								WHERE lang = '$lang'AND lcode = '$big_code' AND mcode = '$m_code' ORDER BY scode";
					...
					while($sml_row = mysql_fetch_object($sml_result)) {
						...
						$s_condi = $sml_row->condi;
						
						if($s_condi == "1") {
							$s_condi_slct1 = "selected";
							$s_condi_slct2 = "";
							$s_condi_slct3 = "";
						} else if($s_condi == "2") {
							$s_condi_slct1 = "";
							$s_condi_slct2 = "selected";
							$s_condi_slct3 = "";
						} else if($s_condi == "3") {
							$s_condi_slct1 = "";
							$s_condi_slct2 = "";
							$s_condi_slct3 = "selected";
						}
				
						echo ("
						<form name='formSU1' method='post' action='$home/process_catg_updS.php'>
						<input type='hidden' name='lang' value='$lang'>
						<input type='hidden' name='gate' value='$login_gate'>
						<input type='hidden' name='m_catg' value='$m_code'>
						<input type='hidden' name='new_scode' value='$s_code'>
						<tr>
							<td height=16>($s_code) <input type=text size=16 name='new_sname' value='$s_name' style='height:24px; WIDTH: 160px'>
								<select name='new_condi' style='height:24px; WIDTH: 110px'>
								<option value='1' $s_condi_slct1>$txt_sys_category_011</option>
								<option value='2' $s_condi_slct2>$txt_sys_category_012</option>
								<option value='3' $s_condi_slct3>$txt_sys_category_013</option>
								</select>
							</td>
							<td align=right>
								<input type='submit' value='$txt_comm_frm12' class='btn btn-default btn-xs'>
								<a href='$home/process_catg_delS.php?lang=$lang&scode=$s_code&gate=$login_gate'>$txt_comm_frm13</a>
							</td>
						</tr>
						</form>");
					}
					...
					echo ("
					<form name='formS1' method='post' action='$home/process_catg_addS.php'>
					<input type='hidden' name='lang' value='$lang'>
					<input type='hidden' name='gate' value='$login_gate'>
					<input type='hidden' name='m_catg' value='$m_code'>
					<tr>
				...
				<td colspan=2>
					...
					while($sml_row = mysql_fetch_object($sml_result)) {
						...
						echo ("
						<form name='formSU2' method='post' action='$home/process_catg_updS.php'>
						<input type='hidden' name='lang' value='$lang'>
						<input type='hidden' name='gate' value='$login_gate'>
						<input type='hidden' name='m_catg' value='$m_code'>
						<input type='hidden' name='new_scode' value='$s_code'>
					...
					if($ss_mode == "add" AND $ss_mcode == $m_code) {
					echo ("
					<form name='formS2' method='post' action='$home/process_catg_addS.php'>
					<input type='hidden' name='lang' value='$lang'>
					<input type='hidden' name='gate' value='$login_gate'>
					<input type='hidden' name='m_catg' value='$m_code'>
					<tr>
