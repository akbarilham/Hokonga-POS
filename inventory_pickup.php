...
$link_list = "$home/inventory_pickup.php?sorting_key=$sorting_key";
$link_upd = "$home/inventory_pickup.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";

$link_act = "$home/inventory_pickup_act.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
?>
...
<?
// Filtering
$sorting_filter = "do_status > '3' AND shop_code2 = '$login_shop'";
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='inventory_pickup.php'>
			<div class='col-sm-2'>
...
for($i = $first; $i <= $last; $i++) {
  ...
  if($mode == "check" AND $do_uid == $uid) {
	...
	$query_qs2 = "SELECT uid,stock,date,flag,pay_num,gudang_code,supp_code,shop_code,org_uid,date,pcode,
				...
				org_pcode,org_barcode FROM shop_product_list_qty WHERE do_num = '$do_num' ORDER BY date ASC";
    ...
    for($qs = 0; $qs < $total_HC; $qs++) {
		  ...
		  $qs_pcode = mysql_result($result_qs2,$qs,10);
			...
			$qs_org_pcode = mysql_result($result_qs2,$qs,22);
			$qs_org_barcode = mysql_result($result_qs2,$qs,23);
	...
	if($qs_sx_num != "") {
		...
		$query_dari = "SELECT uid,pname,product_option1,product_option2,product_option3,product_option4,product_option5,
			price_orgin,dc_rate,save_point,org_pcode,org_barcode FROM shop_product_list WHERE pcode = '$qs_pcode'";
		...
		echo ("
		<form name='signform' method='post' action='inventory_pickup.php'>
        ...
        <tr>
			<td align=right>$dari_org_pcode &nbsp;&nbsp; <i class='fa fa-caret-right'></i></td>
			
            <td align=right>$qs_stock1_K</td>
			<td colspan=3>{$dari_pname} {$dari_product_option1_txt}{$dari_product_option2_txt}</td>
			...
			if($qs_check_date > 0) {
				echo ("<td align=right>
					...
					<input type='text' class='form-control' id='inputSuccess' name='stock_check' value='$qs_stock_check' style='width: 85px; height: 23px; text-align: right; font-size: 1.0em'>
					</span>
				</td>");
			} else {
				echo ("<td align=right>
					<input type='text' class='form-control' name='stock_check' value='$qs_stock' style='width: 85px; height: 23px; text-align: right; font-size: 1.0em'>
				</td>");
			}
			...
			<td>
				<select class='form-control' name='stock_loss_type' style='padding: 3px; width: 85px; height: 23px; font-size: 0.85em'>
...
<?
  ...
  $m_ip = getenv('REMOTE_ADDR');
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_pickup.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=check&do_num=$do_num&uid=$uid'>");
    exit;
