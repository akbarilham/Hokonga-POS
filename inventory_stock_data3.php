...
$link_list = "$home/inventory_stock_data3.php";
$link_list_action = "$home/inventory_stock_data3.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page&mode=upd&view=qty";
$link_post = "$home/inventory_stock_data3.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page";
?>
...
?>
        ...
        <header class="panel-heading">
            <?=$txt_sales_sales_01?> (<?=$txt_sys_shop_071?>)
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search1' method='post' action='inventory_stock_data3.php'>
			<div class='col-sm-2'>
			...
			<form name='search2' method='post' action='inventory_stock_data3.php'>
        ...
        <tr>
			...
			if($keyD) {
				...
				if($keyA) {
					$query_gc = "SELECT count(uid) FROM client_shop WHERE branch_code = '$keyA' AND associate = '0'";
				} else {
					$query_gc = "SELECT count(uid) FROM client_shop WHERE associate = '0'";
				}
				...
				if($keyA) {
					$query_g = "SELECT branch_code,shop_code,shop_name FROM client_shop WHERE branch_code = '$keyA' AND associate = '0' ORDER BY shop_code ASC";
				} else {
					$query_g = "SELECT branch_code,shop_code,shop_name FROM client_shop WHERE associate = '0' ORDER BY branch_code ASC, shop_code ASC";
				}
				...
				for($g = 0; $g < $total_gc; $g++) {
					...
					$g_shop_code = mysql_result($result_g,$g,1);
					$g_shop_name = mysql_result($result_g,$g,2);
					
					$link_list_g = "$link_list?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$g_branch_code&keyB=$keyB&keyC=$keyC&keyD=$g_shop_code&key1=$key1&key2=$key2&key3=$key3";
							
					echo ("<th><a href='$link_list_g' class='tooltips' title=\"\" data-toggle='tooltip' data-placement='top' data-original-title='$g_shop_name'><b>$g_shop_code</b></a></th>");
...
<tr height=22>
   <td colspan=5 align=right><b>Total</b></td>");
	...
	");
				...
				// Total
				$query_gsum = "SELECT sum(stock_end) FROM final_monthly_stock WHERE $sorting_filter AND branch_code = '$g_branch_code' AND date = '$ym1_date'";
				$result_gsum = mysql_query($query_gsum);
					if (!$result_gsum) { error("QUERY_ERROR"); exit; }
				$prd_gsum_now = @mysql_result($result_gsum,0,0);

				
				$query_gsum1 = "SELECT sum(stock) FROM shop_product_list_qty 
				...
				$query_gsum2 = "SELECT sum(stock) FROM shop_product_list_qty 
				...
				for($g = 0; $g < $total_gc; $g++) {
					...
					// SUM
					$query_tsum = "SELECT sum(stock_out) FROM final_monthly_stock WHERE $sorting_filter AND shop_code = '$g_shop_code' AND date = '$ym1_date'";
					...
					$query_tsum1 = "SELECT sum(stock) FROM shop_product_list_qty 
								WHERE $sorting_filter AND flag = 'out' AND shop_code = '$g_shop_code'";
					...
					$query_tsum2 = "SELECT sum(stock) FROM shop_product_list_qty 
								WHERE $sorting_filter AND flag = 'out2' AND shop_code = '$g_shop_code'";
		...
		echo ("<td align=right>{$sum_qty_now_K}</td>");
				...
				for($g = 0; $g < $total_gc; $g++) {
					...
					$query_sum1 = "SELECT sum(stock) FROM shop_product_list_qty 
								WHERE $sorting_filter AND flag = 'out' AND shop_code = '$g_shop_code' AND pcode = '$H_pcode'";
					...
					$query_sum2 = "SELECT sum(stock) FROM shop_product_list_qty 
								WHERE $sorting_filter AND flag = 'out2' AND shop_code = '$g_shop_code' AND pcode = '$H_pcode'";
  ...
  echo("</tr>");
      ...
      if($view == "qty" AND $uid == "$H_uid") {
        ...
        for($qs = 0; $qs < $total_qs; $qs++) {
          ...
          echo ("
          <tr height=22>
            <form name='qs_signform' method='post' action='inventory_stock_data3.php'>
...
<?
	...
	$post_dates = "$post_date1"."$post_date2";
		...
		echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock_data3.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page&mode=upd&view=qty&uid=$new_uid'>");
		exit;
