...
$mmenu = "restore";
$smenu = "restore_stock_opname_monthfb";
...
$link_list = "$home/restore_stock_opname_monthfb.php";
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(org_pcode) FROM table_stock_201507_feelbuy";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(org_pcode) FROM table_stock_201507_feelbuy WHERE $keyfield LIKE '%$key%'";  
}
...
$query_ini = "SELECT count(org_pcode) FROM table_stock_201507_feelbuy";
...
?>
        ...
        <div class="panel-body">
			...
			echo ("
			<form name='search' method='post' action='restore_stock_opname_monthfb.php'>
			<div class='col-sm-3'>
        ...
        <?
		if(!eregi("[^[:space:]]+",$key)) {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_feelbuy ORDER BY $sorting_key $sort_now";
		} else {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_feelbuy WHERE $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
		}
		...
		<div class='col-sm-9'>
				...
				<tr>
				<?	
					$store_branch = "CORP_05";
					$store_branch_name = "FBI";
					$total_ini_store = 211;

					for($num=1; $num<=$total_ini_store; $num++) {
						$num2 = sprintf("%02d", $num);
						
						/*$query_str = "SELECT shop_name FROM table_store_name_cvj WHERE num = '$num2'";
						$result_str = mysql_query($query_str);
							if (!$result_str) { error("QUERY_ERROR"); exit; }
						$new_shop_name = @mysql_result($result_str,0,0);
						
						$query_cd = "SELECT shop_code FROM client_shop WHERE shop_name = '$new_shop_name'";
						...
						echo ("<th colspan=6><div style='width: 100px'><a href='#' class='tooltips' title=\"\" data-toggle='tooltip' data-placement='top' 
							data-original-title='$store_branch_name &gt; $new_shop_name'>$num. $new_shop_code</a></div></th>");
				...
				for($i2 = $first; $i2 <= $last; $i2++) {
							 ...
							 $query2 = "SELECT pnum, org_pcode,stocks
                                FROM table_stock_201507_feelbuy";
...
<?
	...
	echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_monthfb.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
	...
	echo("<meta http-equiv='Refresh' content='10; URL=$home/restore_stock_opname_monthfb.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
