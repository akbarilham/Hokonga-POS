...
?>
        ...
        <header class="panel-heading">
            ...
            <span class="tools pull-right">
				<a href="system_wloc_list_del.php?key_br=<?=$key_br?>&key=<?=$key?>&uid=<?=$loc_uid?>" class="fa fa-trash-o"></a>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="system_wloc_list_upd.php">
								...
								<input type='hidden' name='catg' value='<?=$catg?>'>
									...
									<div class="form-group ">
										...
										<div class="col-sm-9">
											<img src="include/_barcode/html/image.php?code=code39&o=1&dpi=72&t=30&r=2&rot=0&text=<?=$loc_code?>&f1=Arial.ttf&f2=8&a1=&a2=&a3=" border=0>
										</div>
                                    </div>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
											...
											for($st = 0; $st < $total_stc; $st++) {
												...
												if($stf_code == $loc_code_pic) {
													$stf_slct = "selected";
												} else {
													$stf_slct = "";
												}
												
												echo ("<option value='$stf_code' $stf_slct>$stf_name</option>");
											}
											echo ("</select>");
											?>
                                        </div>
...
<?
	...
	if($update_date > 1) {
		$new_upd_date = $update_date;
	} else {
		$new_upd_date = "";
	}

	$query  = "UPDATE wms_location_list SET catg_code = '$new_catg_code', loc_name = '$loc_name', loc_option = '$loc_option', 
			cbm = '$loc_cbm', code_pic = '$code_pic', post_date = '$regis_date', upd_date = '$new_upd_date' WHERE uid = '$loc_uid'"; 
	$result = mysql_query($query);
	if(!$result) { error("QUERY_ERROR"); exit; }
