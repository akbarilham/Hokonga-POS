...
$link_dir = "$home";
$link_list = "$home/crm_mailing2.php";
$link_post = "$home/crm_mailing2_post.php";
$link_upd = "$home/crm_mailing2_upd.php";
$link_del = "$home/crm_mailing2_del.php";
...
if($sorting_key == "corp_name") { $chk3 = "selected"; } else { $chk3 = ""; }
...
// Total Count ---------------------------------------------------- //
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(uid) FROM member_main WHERE $sorting_filter";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(uid) FROM member_main WHERE $sorting_filter AND $keyfield LIKE '%$key%'";  
}
...
?>
        ...
        <div class="panel-body">
			...
			<!--<option value='$PHP_SELF?sorting_key=corp_name&key=$key&keyfield=$keyfield' $chk3>$txt_stf_member_08</option>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,name,gender,id,email,userlevel,signdate,att_table,mb_type,corp_name,mlist,crmflag,memo,birthday FROM member_main 
...
for($i = $first; $i <= $last; $i++) {
   ...
   $att_table = mysql_result($result,$i,7);
   $mb_type = mysql_result($result,$i,8);
   $corp_name = mysql_result($result,$i,9);
   $mlist = mysql_result($result,$i,10);
   $crmflag = mysql_result($result,$i,11);
   $m_mails = mysql_result($result,$i,12);
   $birthday = mysql_result($result,$i,13);
   ...
   $att_table1 = substr($att_table,0,1); // CRM
   $att_table2 = substr($att_table,1,1); // Trade
   $att_table3 = substr($att_table,2,1); // Builder
   $att_table4 = substr($att_table,3,1); // Client
   $att_table5 = substr($att_table,4,1); // Partner
   
   $att_table_half = substr($att_table,0,5);
   ...
   if(!strcmp($key,"$name") && $key) {
    ...
    if($crmflag == "1") {
		echo("<td bgcolor=#FFFFFF align=\"center\"><a href=\"$link_dir/work_mailing_M_remove.php?lang=$lang&gate=$login_gate&uid=$uid2&page=$page&keyfield=$keyfield&key=$key&rmode=2\" onMouseOver=\"status='drop this record'\" onMouseOut=\"status=''\"><font color=#888888>$txt_web_mailing_021</font></a></td>");
	} else {
		echo("<td bgcolor=#FFFFFF align=\"center\"><a href=\"$link_dir/work_mailing_M_add.php?lang=$lang&gate=$login_gate&uid=$uid2&page=$page&keyfield=$keyfield&key=$key&rmode=2\" onMouseOver=\"status='Add this To the Mailing List'\" onMouseOut=\"status=''\"><font color=green>$txt_web_mailing_022</font></a></td>");
	}
	echo("</tr>");
...
?>
			...
			<a href='$link_dir/work_mailing_M_add_all.php?lang=$lang&gate=$login_gate&page=$page&rmode=2'><font color=blue>[ $txt_web_mailing_04 ]</font></a>
			<a href='$link_dir/work_mailing_M_remove_all.php?lang=$lang&gate=$login_gate&page=$page&rmode=2'><font color=red>[ $txt_web_mailing_05 ]</font></a>
	...
	<form method="post2" class="form-horizontal" action="<?echo("$link_send?room=member")?>">		
