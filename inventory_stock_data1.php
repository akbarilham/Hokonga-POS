...
$link_list = "$home/inventory_stock_data1.php";
$link_list_action = "$home/inventory_stock_data1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page&mode=upd&view=qty";
$link_post = "$home/inventory_stock_data1.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page";
?>
...
// Total Count ---------------------------------------------------- //
if(!eregi("[^[:space:]]+",$key)) {
  $query = "SELECT count(uid),sum(stock_org),sum(stock_sell),sum(stock_now) FROM shop_product_list WHERE $sorting_filter1";
} else {
  $encoded_key = urlencode($key);
  $query = "SELECT count(uid),sum(stock_org),sum(stock_sell),sum(stock_now) FROM shop_product_list WHERE $sorting_filter1 AND $keyfield LIKE '%$key%'";  
}
...
$total_stock_org = mysql_result($result,0,1);
	$total_stock_org_K = number_format($total_stock_org);
$total_stock_sell = mysql_result($result,0,2);
	$total_stock_sell_K = number_format($total_stock_sell);
$total_stock_now = mysql_result($result,0,3);
	$total_stock_now_K = number_format($total_stock_now);
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search1' method='post' action='inventory_stock_data1.php'>
			<div class='col-sm-2'>
			...
			<form name='search2' method='post' action='inventory_stock_data1.php'>
...
// Stock in WHs
	...
	if(!preg_match("[^[:space:]]+/i",$key)) {
		$query_tsmW1 = "SELECT sum(stock) FROM shop_product_list_qty WHERE $sorting_filter AND flag = 'in'";
	} else {
		$query_tsmW1 = "SELECT sum(stock) FROM shop_product_list_qty WHERE $sorting_filter AND flag = 'in' AND $keyfield LIKE '%$key%'";
	}
	...
	if(!preg_match("[^[:space:]]+/i",$key)) {
		$query_tsmW2 = "SELECT sum(stock) FROM shop_product_list_qty WHERE $sorting_filter AND flag = 'out'";
	} else {
		$query_tsmW2 = "SELECT sum(stock) FROM shop_product_list_qty WHERE $sorting_filter AND flag = 'out' AND $keyfield LIKE '%$key%'";
	}
...
// Total
$prd_tsm_qty_now = $prd_tsmW1_qty_in - $prd_tsmW2_qty_now_K;
	$prd_tsm_qty_now_K = number_format($prd_tsm_qty_now);
	
$prd_grand_total_stock = $prd_tsm_qty_now + $prv_stock_end;
	$prd_grand_total_stock_K = number_format($prd_grand_total_stock);
 
echo ("
<tr height=22>
   <td colspan=5 align=right><b>$txt_invn_stockin_71</b></td>
   <td align=right></td>
   <td align=right></td>
   <td colspan=2 align=right>$prv_stock_end_K</td>
</tr>
<tr height=22>
   <td colspan=5 align=right><b>$txt_invn_stockin_74</b></td>
   <td align=right>$prd_tsmW1_qty_in_K</td>
   <td align=right>$prd_tsmW2_qty_out_K</td>
   <td colspan=2 align=right>$prd_tsmW2_qty_now_K</td>
</tr>
<tr height=22>
   <td colspan=5 align=right><b>$txt_invn_stockin_72</b></td>
   <td align=right></td>
   <td align=right></td>
   <td colspan=2 align=right>$prd_grand_total_stock_K</td>
</tr>
");
...
echo ("
<tr height=22>
   <td colspan=5 align=right><b>$txt_sys_shop_079</b></td>
   <td align=right>{$prd_tsmW1_qty_in_K}</td>
   <td align=right>{$prd_tsmW2_qty_out_K}</td>
   <td colspan=2 align=right>{$prd_tsmW2_qty_now_K}</td>
</tr>
...
echo ("
<tr height=22>
   <td colspan=5 align=right><b>Shop/Store</b></td>
...
// Total from Product List

if($prd_tsm_qty_now == $total_stock_now) {
	$stock_font_clr = "blue";
} else {
	$stock_font_clr = "red";
}
...
echo ("
<tr height=22>
   <td colspan=5 align=right><b>= Source</b></td>
   <td align=right><font color='$stock_font_clr'>$total_stock_org_K</font></td>
   <td align=right><font color='$stock_font_clr'>$total_stock_sell_K</font></td>
   <td colspan=2 align=right><font color='$stock_font_clr'>$total_stock_now_K</font></td>
</tr>
");
*/
...
for($i = $first; $i <= $last; $i++) {
   ...
   $H_gcode = mysql_result($result,$i,11);
	...
	// Stock of Begining
	$query_sub1 = "SELECT sum(stock_end) FROM final_monthly_stock WHERE pcode = '$H_pcode' AND $sorting_filter AND date = '$ym1_date'";
  ...
  echo("</tr>");
      ...
      if($view == "qty" AND $uid == "$H_uid") {
		...
		$query_qs1 = "SELECT sum(stock_end) FROM final_monthly_stock WHERE pcode = '$H_pcode' AND $sorting_filter AND date = '$ym1_date' ORDER BY uid DESC";
        $result_qs1 = mysql_query($query_qs1,$dbconn);
        if (!$result_qs1) { error("QUERY_ERROR"); exit; }   
        ...
        $query_qs2 = "SELECT uid,stock,date,flag,pay_num,gudang_code,supp_code,shop_code,org_uid,date 
                      FROM shop_product_list_qty WHERE pcode = '$H_pcode' AND $sorting_filter ORDER BY date ASC, flag ASC, shop_code ASC";
        ...
        for($qs = 0; $qs < $total_qs; $qs++) {
          ...
          echo ("
          <tr height=22>
            <form name='qs_signform1' method='post' action='inventory_stock_data1.php'>
            ...
            <form name='qs_signform2' method='post' action='inventory_stock_data1.php'>
...
<?
	...
	$post_dates = "$post_date1"."$post_date2";
		...
		echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock_data1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page&mode=upd&view=qty&uid=$new_uid'>");
		exit;
