...
$link_list = "inventory_stock1D.php?sorting_key=$sorting_key";
$link_list_action = "inventory_stock1D.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";

$link_post = "inventory_stock1D.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
$link_post_catg = "inventory_stock1D.php?mode=post_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
...
?>
        ...
        <tr>
			...
			<th colspan=2><?=$txt_invn_stockin_25?></th>
        </tr>
        </thead>
...
<?
echo ("
<form name='p_cart_dtl' method='post' action='inventory_purchase_cart2d.php'>
<input type=hidden name='cart_mode' value='CART_LOOP'>
...
<tr>
   ...
   <td colspan=2 align=right>{$prd_tsm_price_K}</td>
</tr>
");
...
for($i = $first; $i <= $last; $i++) {
  ...
  echo("<td bgcolor='$highlight_color' align=right><font color='navy'>$prd_stock_now_K</font></td>");
	...
	echo ("
	<td bgcolor='$highlight_color'><input type='text' name='new_cart_qty[$prd_uid]' value='$H2_qty' maxlength=6 value='10' 
	...
	<td bgcolor='$highlight_color'><select name='unpack_unit_set[$prd_uid]' style='width:50px; height: 20px; text-align: center'>
...
?>
		...
		<div class='form-group'>
			<div class='col-sm-3'>
				<input type="submit" value="Add this Page into Purchasing List" class="btn btn-primary">
			</div>
		</div>
...
<?
  ...
  if($add_mode == "LIST_DEL") {
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?mode=upd_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_catg_uid&page=$page&mode2=$mode2&gcode=$gcode'>");
    exit;
  ...
  // 상품 수정 및 추가
      ...
      if($add_stock_chk == "3") { // 추가 출고
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=upd&uid=$new_uid&mode2=$mode2&gcode=$gcode'>");
          exit;
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=post&uid=$my_catg_uid&mode2=$mode2&gcode=$gcode'>");
          exit;
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page&mode2=$mode2&gcode=$gcode'>");
          exit;
  ...
  // 상품 입력 -------------------------------------------------------------------------------------------// 
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode2=$mode2&gcode=$gcode'>");
    exit;
...
echo("<meta http-equiv='Refresh' content='0; URL=inventory_stock1D.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page&catg=$catg&view=qty&mode2=$mode2&gcode=$gcode'>");
exit;
