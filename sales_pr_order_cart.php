...
if($cart_mode == "CART_NEW") {
      ...
      $query_C2 = "INSERT INTO shop_cart (uid,prd_uid,branch_code,f_class,user_id,user_ip,
			...
			values ('','','$login_branch','ib2b','$login_id','$m_ip','','$cart_qty','0','$post_dates',
			'$new_cart_pname','$new_cart_price_orgin')";
      $result_C2 = mysql_query($query_C2);
      if (!$result_C2) { error("QUERY_ERROR"); exit; }
      
      // 주문 리스트로 돌아가기
      echo("<meta http-equiv='Refresh' content='0; URL=$home/sales_pr_order.php?sorting_key=$sorting_key&key_shop=$key_shop&keyfield=$keyfield&key=$key&po_currency=$po_currency'>");
      exit;
      ...
      // 상품 리스트로 돌아가기
	  echo("<meta http-equiv='Refresh' content='0; URL=$home/sales_pr_order.php'>");
      exit;
      ...
      $new_fname = "Ref. $new_po_num";
		...
		$query_E1 = "SELECT branch_code,gate FROM client_shop WHERE shop_code = '$new_shop_code' ORDER BY shop_code DESC";
        $result_E1 = mysql_query($query_E1);
            if (!$result_E1) { error("QUERY_ERROR"); exit; }
        $E_branch_code = @mysql_result($result_E1,0,0);
		$E_gate = @mysql_result($result_E1,0,1);

	  
      $query_P2 = "INSERT INTO shop_purchase (uid,branch_code,gate,client_code,manager_code,f_class,po_num,
            po_qty,currency,po_tamount,order_date) values ('','$E_branch_code','$E_gate','$new_shop_code','$login_id','in','$new_po_num',
      ...
      // payment collection
		$query_P2 = "INSERT INTO shop_payment (uid,branch_code,gate,shop_code,f_class,pay_num,pay_amount,pay_amount_money,
			pay_amount_point,pay_amount_delivery,order_date) values ('','$E_branch_code','$E_gate','$new_shop_code',
      ...
      $result_P3 = mysql_query("UPDATE shop_cart SET expire = '1', order_num = '$new_po_num' 
                  WHERE user_id = '$login_id' AND f_class = 'b2b' AND expire = '0'",$dbconn);
