...
// 발주 정보 테이블
$pm_query = "SELECT uid,branch_code,po_num,po_qty,po_tamount,client_code,order_date,currency,do_port2,po_ppn FROM shop_purchase WHERE uid = '$P_uid'";
...
$pm_do_port2 = @mysql_result($pm_result,0,8);
$pm_ppn = @mysql_result($pm_result,0,9);

// PORT
$query_port2 = "SELECT port_name,port_area FROM code_port WHERE port_code = '$pm_do_port2' ORDER BY uid DESC";
$result_port2 = mysql_query($query_port2);
	if (!$result_port2) { error("QUERY_ERROR"); exit; }
$port_name2 = @mysql_result($result_port2,0,0);
$port_area2 = @mysql_result($result_port2,0,1);
...
if($pm_currency == "USD") {
	...
	$amount = $pm_po_amount + $pm_ppn;
	$pm_po_amount_K = number_format($amount);
	$pm_currency_tag = "Rp.";
}
...
// 상세 발주 정보
$query_HC = "SELECT count(uid) FROM shop_cart WHERE order_num = '$pm_po_num' AND f_class = 'out' AND expire = '1'";
        ...
        <div class="panel">
                ...
                <div class="invoice-address">
                    <div class="row">
                        <div class="col-md-5 col-sm-5 col-xs-5">
                            <h4 class="inv-to">Purchase Order To</h4>
                        ...
                        <div class="col-md-4 col-md-offset-3 col-sm-4 col-sm-offset-3 col-xs-4 col-xs-offset-3">
                            <div class="inv-col"><span>PO No. :</span> <?=$pm_po_num?></div>
                            <div class="inv-col"><span>PO Date :</span> <?=$signdate_txt?></div>
            ...
            <table class="table table-bordered table-invoice">
				...
				$query_H = "SELECT uid,prd_uid,pcode,qty,product_color,product_size,p_option1,p_option2,p_option3,p_option4,p_option5,
							...
							WHERE order_num = '$pm_po_num' AND f_class = 'out' AND expire = '1' ORDER BY date ASC";
				...
				for($h = 0; $h < $total_HC; $h++) {
					...
					// 상품명, 상품별 결제액
					$query_dari = "SELECT uid,pname,price_orgin,tprice_orgin,stock_now,stock_sell,unit FROM shop_product_list WHERE uid = '$H_prd_uid'";
					...
					$p_ppn = (10 / 100) * $p_total_price;
					// echo 'total_price '.$p_total_price;
					// echo '<br/>ppn_price'.$p_ppn;
					if($pm_currency == "USD") {
						$p_total_price_K = number_format($p_total_price,2);
					} else {
						$p_total_price_K = number_format($p_total_price);
						$p_ppn_K = number_format($p_ppn);
					}
					...
					<tr>
						<td>$cart_no</td>
						<td>
							<h4>[$H_org_pcode] $dari_pname</h4>
							<p>{$H_p_opt1_txt}{$H_p_opt2_txt}{$H_p_opt3_txt}{$H_p_opt4_txt}{$H_p_opt5_txt}</p>
						</td>
						<td><strong>$dari_amount2R_K</strong></td>
						<td><strong>$H2_qty $dari_unit_reset</strong></td>
						<!--<td class='text-center'><strong>$H_qty_K</strong></td>-->
						<td class='text-center'><strong>$H_cbm</strong></td>
						<td><strong>$dari_tamount_K</strong></td>
					</tr>");
				...
				#$p_tax = "0";
				$p_grand_total = $p_total_price + $p_ppn;
				$p_grand_total_no_ppn = $p_total_price;
					$p_grand_total_k = number_format($p_grand_total);
					$p_grand_total_no_ppn_k = number_format($p_grand_total_no_ppn);

				#echo 'type of ppn'.$pm_ppn;
				?>
				

                <tr>
                    <td colspan="2" class="payment-method">
                        <!--<h4>Arrival: <?=$port_name2?>, <?=$port_area2?></h4>-->
                    ...
                    <td colspan="3" class="text-right">
                        <p style="text-align:left; width:115px; margin-top:0px;">Sub Total</p>
						<p style="text-align:left; width:115px; margin-top:0px;">Tax (PPN)</p>
                        <p>&nbsp;</p>
                        <p style="text-align:left; width:115px; margin-top:0px;"><strong>GRAND TOTAL</strong></p>
                    </td>
					<!--
					...
					?>
                    <td class="text-center">
                        <p style="text-align:left; width:115px; margin-top:0px;"><?=$p_total_price_K?></p>
                        <p style="text-align:left; width:115px; margin-top:0px;">0</p>
                        <p style="text-align:left; width:115px; margin-top:0px;">&nbsp;</p>
                        <p style="text-align:left; width:115px; margin-top:0px;"><strong><?=$p_grand_total_no_ppn_k?></strong></p>
                    </td>
                    <?
                	} else {
                    ?>					
                    <td class="text-center">
                        <p style="text-align:left; width:115px; margin-top:0px;"><?=$p_total_price_K?></p>
                        <p style="text-align:left; width:115px; margin-top:0px;"><?=$p_ppn_K?></p>
                        <p>&nbsp;</p>
                        <p style="text-align:left; width:115px; margin-top:0px;"><strong><?=$p_grand_total_k?></strong></p>
                    </td>
                    <? } ?>
                </tr>
