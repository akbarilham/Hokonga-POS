...
$link_list = "$home/finance_cost.php?sorting_key=$sorting_key";
$link_upd = "$home/finance_cost.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
?>
...
// Filtering
if($login_level > "2") {
  $sorting_filter = "branch_code = '$login_branch' AND f_class = 'out' AND f_paylink = '0'";
  $sorting_filter_G = "branch_code = '$login_branch' AND userlevel < '6'";
} else {
  $sorting_filter = "branch_code = '$login_branch' AND gate = '$login_gate' AND f_class = 'out' AND f_paylink = '0'";
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='finance_cost.php'>
			<div class='col-sm-2'>
...
?>
				...
				<a href="<?echo("$link_upd&mode=request")?>"><input type="button" value="<?=$txt_fin_cost_01?>" class="btn btn-primary"></a>
      ...
      echo ("
      <form name='signform2' class='cmxform form-horizontal adminex-form' method='post' action='finance_cost.php'>
	  ...
	  <? } else if($mode == "request") { ?>
		...
		<?
		echo ("
		<form name='signform' class='cmxform form-horizontal adminex-form' method='post' action='finance_cost.php'>
        ...
        <div class="panel-body">
									...
									<div class='form-group'>
                                        <div class='col-sm-offset-3 col-sm-9'>
											<input class='btn btn-primary' type='submit' value='$txt_fin_cost_01'>
										</div>
                                    </div>");
									?>
...
<?
  ...
  $new_pay_num = "GE-"."$exp_branch_code"."-"."$signdate";
  ...
  $new_fcatg = substr($new_acc_code,0,2);
    ...
    // 리스트로 되돌아 가기
    echo("<meta http-equiv='Refresh' content='0; URL=$home/finance_cost.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page'>");
    exit;
  ...
  $new_fcatg = substr($new_acc_code,0,2);
    ...
    $query_F2 = "INSERT INTO finance (uid,branch_code,gate,f_class,f_paylink,f_catg,f_code,f_name,f_remark,currency,
        pay_type,amount,post_date,process,bank_name,remit_code,pay_bank,pay_card) values ('',
        '$login_branch','$new_shop_code','out','0','$new_fcatg','$new_acc_code','$new_fname','$new_fremark','$new_currency',
    ...
    // 리스트로 되돌아 가기
    echo("<meta http-equiv='Refresh' content='0; URL=$home/finance_cost.php?mode=check&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page'>");
    exit;
