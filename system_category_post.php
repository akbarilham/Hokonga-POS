...
<section id="container" class="">
	...
	<section id="main-content">
		...
		<?
		echo ("
		<form name='signform1' method='post' action='system_category_post.php'>
...
<?
  	...
  	if($b_code == "add") {
	    ...
	    $query_L1  = "INSERT INTO shop_catgbig (lcode, lname, lang, gate) VALUES ('$add_code','$b_name','en','host')";
	    $result_L1 = mysql_query($query_L1,$dbconn);
	    if(!$result_L1) { error("QUERY_ERROR"); exit; }
	
	    $query_L2  = "INSERT INTO shop_catgbig (lcode, lname, lang, gate) VALUES ('$add_code','$b_name','ko','host')";
	    $result_L2 = mysql_query($query_L2,$dbconn);
	    if(!$result_L2) { error("QUERY_ERROR"); exit; }
	
	    $query_L3  = "INSERT INTO shop_catgbig (lcode, lname, lang, gate) VALUES ('$add_code','$b_name','in','host')";
...
<section id="container" class="">
	...
	<section id="main-content">
		...
		<form name='signform2' method='post' action='system_category_post.php'>
		<input type='hidden' name='step_next' value='permit_okay'>
		<input type='hidden' name='gate' value='$login_gate'>
		...
		$mid_query = "SELECT * FROM shop_catgmid WHERE lang = '$lang' AND lcode = '$b_code' ORDER BY mcode";
...
<?
	...
	$b_code = substr($addcode, 0,2); // 대분류 코드를 얻는다.
	$m_name = addslashes($m_name);

	// 중분류 항목을 추가 (3개 언어 동시에)
	$query_M1  = "INSERT INTO shop_catgmid (lcode, mcode, mname, lang, gate) VALUES ('$b_code','$addcode','$m_name', 'en', 'host')";
	$result_M1 = mysql_query($query_M1);
		if(!$result_M1) {	error("QUERY_ERROR");	exit;	}

	$query_M2  = "INSERT INTO shop_catgmid (lcode, mcode, mname, lang, gate) VALUES ('$b_code','$addcode','$m_name', 'ko', 'host')";
	$result_M2 = mysql_query($query_M2);
		if(!$result_M2) {	error("QUERY_ERROR");	exit;	}
	
	$query_M3  = "INSERT INTO shop_catgmid (lcode, mcode, mname, lang, gate) VALUES ('$b_code','$addcode','$m_name', 'in', 'host')";
