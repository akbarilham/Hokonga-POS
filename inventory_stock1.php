...
$link_list = "$home/inventory_stock1.php?sorting_key=$sorting_key";
$link_list_action = "$home/inventory_stock1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";

$link_post = "$home/inventory_stock1.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
$link_post_catg = "$home/inventory_stock1.php?mode=post_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
...
?>
        ...
        <header class="panel-heading">
            <?=$hsm_name_04_061?> &nbsp;&nbsp; 
			<a href='inventory_stock1D.php'><i class='fa fa-list-ul'></i></a>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='inventory_stock1.php'>
			<div class='col-sm-2'>
...
for($i = $first; $i <= $last; $i++) {
  ...
  echo("
        <td bgcolor='$highlight_color'><a href='$link_list_action&mode2=dtl&gcode=$prd_gcode'>{$prd_name}</a></td>
  ...
  if($mode2 == "dtl" AND $gcode == $prd_gcode) {
    ...
    $query_H = "SELECT uid,gate,catg_code,pcode,pname,
      ...
      product_option4,product_option5,price_orgin,org_pcode FROM shop_product_list WHERE gcode = '$prd_gcode' 
    ...
    <tr>
      ...
      <td bgcolor='$highlight_color_L'><a href='$link_list_action&mode=upd&uid=$H_prd_uid&mode2=dtl&gcode=$prd_gcode' 
            data-toggle='tooltip' data-placement='top' title='[$H_prd_code] {$H_prd_name} - {$H_p_opt1_txt}{$H_p_opt2_txt}{$H_p_opt3_txt}{$H_p_opt4_txt}{$H_p_opt5_txt}'>[$H_org_pcode] {$H_prd_name} 
...
// Data Extraction
          $query_upd = "SELECT uid,catg_uid,gudang_code,supp_code,branch_code,gate,shop_code,catg_code,gcode,pcode,org_pcode,org_barcode,pname,
                        ...
                        confirm_status,post_date,upd_date,lot_no,date_expire,product_desc,cbm FROM shop_product_list WHERE uid = '$uid'";
...
?>
			...
			<div class='col-sm-2'>
				...
				<form name='p_cart' method='post' action='inventory_purchase_cart.php'>
				...
				<input type=hidden name='cart_org_pcode' value='$upd_org_pcode'>
				<input type=hidden name='cart_org_barcode' value='$upd_org_barcode'>
				...
				<input type=hidden name='cart_cbm' value='$upd_cbm'>
			...
			<div class='col-sm-2'>
				...
				<select name='unpack_unit_set' class='form-control'>
				<option value=\"\">EA</option>");
			
				$query_u1 = "SELECT count(uid) FROM shop_product_catg_unit WHERE gcode = '$upd_gcode'";
				$result_u1 = mysql_query($query_u1,$dbconn);
					if (!$result_u1) { error("QUERY_ERROR"); exit; }
      
				$total_unit = @mysql_result($result_u1,0,0);
      
				$query_u2 = "SELECT uid,unit_name,unit_qty FROM shop_product_catg_unit WHERE gcode = '$upd_gcode' ORDER BY unit_name ASC"; 
			...
			<div class='col-sm-2'>
				<input type='submit' value='$txt_invn_purchase_12' class='btn btn-primary'>
				</form>
			</div>");
		...
		<form name='signform' class="cmxform form-horizontal adminex-form" method='post' action='inventory_stock1.php'>
		...
		<?
          if($mode == "del" AND $uid) { // Cart

          $query_upd = "SELECT uid,gudang_code,supp_code,branch_code,gate,catg_code,pcode,pname,
                        price_orgin,price_market,price_sale,stock_org,catg_uid FROM shop_product_list WHERE uid = '$uid'";
          $result_upd = mysql_query($query_upd);
          if(!$result_upd) { error("QUERY_ERROR"); exit; }
          $row_upd = mysql_fetch_object($result_upd);
...
<?
  ...
  if($add_mode == "LIST_DEL") {
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?mode=upd_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_catg_uid&page=$page&mode2=$mode2&gcode=$gcode'>");
    exit;
  ...
  // 상품 수정 및 추가
      ...
      if($add_stock_chk == "3") { // 추가 출고
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=upd&uid=$new_uid&mode2=$mode2&gcode=$gcode'>");
          exit;
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode=post&uid=$my_catg_uid&mode2=$mode2&gcode=$gcode'>");
          exit;
          ...
          echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page&mode2=$mode2&gcode=$gcode'>");
          exit;
  ...
  // 상품 입력 -------------------------------------------------------------------------------------------// 
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page&mode2=$mode2&gcode=$gcode'>");
    exit;
...
echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_stock1.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page&catg=$catg&view=qty&mode2=$mode2&gcode=$gcode'>");
exit;
