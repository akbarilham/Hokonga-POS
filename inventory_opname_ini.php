...
$link_list = "$home/inventory_opname_ini.php";
$link_list_action = "$home/inventory_opname_ini.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&key1=$key1&key2=$key2&key3=$key3&page=$page&mode=upd&view=qty";

$link_post = "$home/inventory_opname_ini.php?mode=post&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&key1=$key1&key2=$key2&key3=$key3&page=$page";
$link_post_catg = "$home/inventory_opname_ini.php?mode=post_catg&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&key1=$key1&key2=$key2&key3=$key3&page=$page";
?>
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search1' method='post' action='inventory_opname_ini.php'>
			<div class='col-sm-2'>
...
echo ("
<tr height=22>
   <td colspan=4 align=right><b>$txt_invn_gudang_01</b></td>
...
echo ("
<tr height=22>
   <td colspan=4 align=right><b>$txt_sys_shop_01</b></td>
   ...
   $H_gcode = mysql_result($result,$i,11);
	...
	<tr>
		...
		<td>{$H_pname}</td>
			...
			<form name='qs_signform' method='post' action='inventory_opname_ini.php'>
...
<?
	...
	// Stocks in Shops
	$query_cnt1 = "SELECT count(uid),sum(stock) FROM shop_product_list_qty WHERE flag = 'out' AND pcode = '$li_pcode'";
	$result_cnt1 = mysql_query($query_cnt1);
		if (!$result_cnt1) { error("QUERY_ERROR"); exit; }
	$qty_cnt1 = @mysql_result($result_cnt1,0,0);
	$qty_sum1 = @mysql_result($result_cnt1,0,1);
	
	$query_uid1 = "SELECT uid FROM shop_product_list_qty WHERE flag = 'out' AND pcode = '$li_pcode' ORDER BY pcode DESC";
	$result_uid1 = mysql_query($query_uid1);
		if (!$result_uid1) { error("QUERY_ERROR"); exit; }
	$max_uid1 = @mysql_result($result_uid2,0,0);
	
	$query_cnt1dt = "SELECT count(uid),sum(stock) FROM shop_product_list_qty WHERE flag = 'out2' AND pcode = '$li_pcode' AND shop_code = '$keyD'";
	...
	$query_uid1dt = "SELECT uid FROM shop_product_list_qty WHERE flag = 'out2' AND pcode = '$li_pcode' AND shop_code = '$keyD' ORDER BY pcode DESC";
	...
	echo("<meta http-equiv='Refresh' content='0; URL=$home/inventory_opname_ini.php?mode=upd&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&keyA=$keyA&keyB=$keyB&keyC=$keyC&keyD=$keyD&key1=$key1&key2=$key2&key3=$key3&page=$page'>");
	exit;
