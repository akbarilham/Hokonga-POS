...
?>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="restore_stock_opname_fb.php">
								...
								<input type='hidden' name='mb_type' value='<?=$mb_type?>'>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="data_name" type="text" value="table_stock_201507_feelbuy" />
                                        </div>
                                    </div>
...
<?
	...
	if($job_slct == "4") {
		...
		echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_fb.php'>");
		exit;
		...
		$queryC2 = "SELECT count(pnum) FROM table_stock_201507_feelbuy";
		$resultC2 = mysql_query($queryC2);
			if (!$resultC2) { error("QUERY_ERROR"); exit; }
		$total_recordC2 = @mysql_result($resultC2,0,0);

		$queryD2 = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_feelbuy ORDER BY pnum ASC";
		$resultD2 = mysql_query($queryD2);
		if (!$resultD2) { error("QUERY_ERROR"); exit; }
				...
				for($o1=0; $o1<211; $o1++) {
					...
					# if($sp_opt1[$output] > 0) {
						...
						*/
							...
							$query_shop = "SELECT shop_code FROM table_store_name_fbi WHERE num = '$num'";
							$result_shop = mysql_query($query_shop);
							if (!$result_shop) { error("QUERY_ERROR"); exit; }
							$new_shop_code = @mysql_result($result_shop,0,0);

							// Shop List
							$query_sh = "SELECT shop_code,branch_code,associate,group_code FROM client_shop WHERE shop_code= '$new_shop_code' and branch_code='CORP_05'";
