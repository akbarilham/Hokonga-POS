...
$link_list = "$home/hr_member_work.php";
          ...
          <section class="wrapper">
             ...
             <span class="tools pull-right">
              ...
              <div class="panel-body">
				...
				<form name='search_date' method='post' action='hr_member_work.php'>
...
for($i = $first; $i < $total_record; $i++) {
  ...
  if($userlevel == "0") {
	...
	// Work
	$query_Hc = "SELECT count(uid) FROM member_staff_worktime WHERE code = '$user_code' AND date = '$p_date_sets' ORDER BY uid DESC";
    $result_Hc = mysql_query($query_Hc);
		if (!$result_Hc) {   error("QUERY_ERROR");   exit; }
    $H_count = @mysql_result($result_Hc,0,0);
	
	$query_H = "SELECT uid,nik,work_status,work_off_type,work_time1,work_time2,work_in,work_out,off_report,off_report_why,warning,
				work_in_early,work_in_late,work_out_early,work_hours
				FROM member_staff_worktime WHERE code = '$user_code' AND date = '$p_date_sets' ORDER BY uid DESC";
	...
	$H_work_status = @mysql_result($result_H,0,2);
	$H_work_off_type = @mysql_result($result_H,0,3);
	$H_work_time1 = @mysql_result($result_H,0,4);
	$H_work_time2 = @mysql_result($result_H,0,5);
	$H_work_in = @mysql_result($result_H,0,6);
	$H_work_out = @mysql_result($result_H,0,7);
	$H_off_report = @mysql_result($result_H,0,8);
	$H_off_report_why = @mysql_result($result_H,0,9);
	$H_warning = @mysql_result($result_H,0,10);
	$H_work_in_early = @mysql_result($result_H,0,11);
	$H_work_in_late = @mysql_result($result_H,0,12);
	$H_work_out_early = @mysql_result($result_H,0,13);
	$H_work_hours = @mysql_result($result_H,0,14);
	...
	echo ("
	<form name='signform1' method='post' action='hr_member_work.php'>
...
<?
	...
	if($step_action == "post") {
			$query_add = "INSERT INTO member_staff_worktime (uid,branch_code,code,name,nik,date,shift,
				work_status,work_off_type,work_time1,work_time2,work_in,work_out,
				work_in_early,work_in_late,work_out_early,work_hours,warning)
				VALUES ('','$new_branch_code','$new_user_code','$new_user_name','$new_user_nik','$new_date_sets','$new_shift',
				'$new_work_status','$new_work_off_type','$work_time1','$work_time2','$work_in','$work_out',
				'$new_work_in_early','$new_work_in_late','$new_work_out_early','$new_work_hours','$warning')";
			$result_add = mysql_query($query_add);
			if(!$result_add) { error("QUERY_ERROR"); exit; }
  
	} else if($step_action == "update") {
			$result2 = mysql_query("UPDATE member_staff_worktime SET nik = '$new_user_nik', work_status = '$new_work_status', 
				work_off_type = '$new_work_off_type', work_time1 = '$work_time1', work_time2 = '$work_time2', work_in = '$work_in', work_out = '$work_out', 
				work_in_early = '$new_work_in_early', work_in_late = '$new_work_in_late', work_out_early = '$new_work_out_early', 
				work_hours = '$new_work_hours', warning = '$warning' WHERE uid = '$new_uid'");
			if (!$result2) { error("QUERY_ERROR"); exit;	}
	}
  

  echo("<meta http-equiv='Refresh' content='0; URL=$home/hr_member_work.php?hr_type=$hr_type&hr_dir=$hr_dir&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page'>");
  exit;
