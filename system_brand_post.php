...
?>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="system_brand_post.php">
								...
								<input type="hidden" name="user_id" value="<?=$login_id?>">
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="brand_name" type="text" required />
                                        </div>
                                    </div>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="brand_origin" type="text" />
                                        </div>
                                    </div>
...
<?
  ...
  // 코드 자동부여
  $rm_query = "SELECT brand_code FROM shop_brand ORDER BY brand_code DESC";
  $rm_result = mysql_query($rm_query);
  if (!$rm_result) { error("QUERY_ERROR"); exit; }
  $max_room = @mysql_result($rm_result,0,0);
	$new_room_num1 = $max_room + 1;
 
  $new_room_num = sprintf("%02d", $new_room_num1); // 2자리 수
  
  
  if($max_room == "") {
    $new_brand_code = "01";
  } else {
    $new_brand_code = "$new_room_num";
  }
  ...
  if($pin_key) {
    ...
    $query_M1 = "INSERT INTO shop_brand (uid,brand_code,brand_name,brand_origin,brand_show,lang,gate,branch_code) 
          values ('','$new_brand_code','$brand_name','$brand_origin','1','$lang','$login_gate','$new_branch_code')";
