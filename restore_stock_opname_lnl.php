...
$mmenu = "restore";
$smenu = "restore_stock_opname_lnl";
...
?>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="restore_stock_opname_lnl.php">
								...
								<input type='hidden' name='mb_type' value='<?=$mb_type?>'>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
                                            <input class="form-control" id="cname" name="data_name" type="text" value="summary_lnl_2015" />
                                        </div>
                                    </div>
...
<?
	...
	if($job_slct == "4") {
		...
		echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_lnl.php'>");
		exit;
		...
		$queryC2 = "SELECT count(pnum) FROM summary_lnl_2015";
		$resultC2 = mysql_query($queryC2);
			if (!$resultC2) { error("QUERY_ERROR"); exit; }
		$total_recordC2 = @mysql_result($resultC2,0,0);

		$queryD2 = "SELECT pnum,org_pcode,stocks FROM summary_lnl_2015 ORDER BY pnum ASC";
		...
		$queryA1 = "SELECT count(num) FROM table_store_name_lnl";
		...
		$count1 = @mysql_result($resultA1,0,0);
  		...
  		for($o1=0; $o1<78; $o1++) {
	  		$num = $o1 + 1; 
	  		$queryA2 = "SELECT shop_code,shop_name FROM table_store_name_lnl where num = '$num' order by shop_code ASC";
			$resultA2 = mysql_query($queryA2);
				if (!$resultA2) { error("QUERY_ERROR"); exit; }
			$shop = @mysql_result($resultA2,0,0);
			$nameshop = @mysql_result($resultA2,0,1);

			$queryC2 = "SELECT count(shop_code),shop_code FROM summary_lnl_2015 where shop_code = '$shop' group by shop_code asc";
			$resultC2 = mysql_query($queryC2);
				if (!$resultC2) { error("QUERY_ERROR"); exit; }
			$count2 = @mysql_result($resultC2,0,0);
			$shop2 = @mysql_result($resultC2,0,1);

			$queryD2 = "SELECT pnum,org_pcode,shop_code,stocks,actual_balance FROM summary_lnl_2015 WHERE shop_code = '$shop' ORDER BY shop_code ASC";
			$resultD2 = mysql_query($queryD2);
				if (!$resultD2) { error("QUERY_ERROR"); exit; }
			...
			echo 'Item ===['.$count2.']=== Shop Code : ['.$shop2.']|['.$nameshop.'] </br>';
				...
				for($ra=0; $ra < $count2; $ra++){		
					...
					$query_sh = "SELECT shop_code,branch_code,associate,group_code FROM client_shop WHERE shop_code= '$shop' and branch_code='CORP_04'";
