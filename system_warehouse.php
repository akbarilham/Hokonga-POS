...
// Total Count ---------------------------------------------------- //
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(uid) FROM code_gudang";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(uid) FROM code_gudang WHERE $keyfield LIKE '%$key%'";  
}
...
?>
        ...
        <tr>
            ...
            <th><?=$txt_invn_gudang_05?></th>
            <th><?=$txt_invn_gudang_06?></th>
			<th><?=$txt_invn_gudang_07?></th>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,branch_code,gudang_code,gudang_name,manager,userlevel,signdate
    FROM code_gudang ORDER BY gudang_code ASC";
} else {
   $query = "SELECT uid,branch_code,gudang_code,gudang_name,manager,userlevel,signdate
    FROM code_gudang WHERE $keyfield LIKE '%$key%' ORDER BY gudang_code ASC";
}
...
for($i = 0; $i < $total_record; $i++) {
   ...
   $gudang_code = mysql_result($result,$i,2);
   $gudang_name = mysql_result($result,$i,3);
   $gudang_manager = mysql_result($result,$i,4);
  ...
  echo("<td><a href='$link_upd?uid=$uid'>$gudang_code</a></td>");
  echo("<td><a href='$link_upd?uid=$uid'>$gudang_name</a></td>");
    
  echo("<td>$gudang_manager</td>");
