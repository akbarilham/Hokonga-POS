...
$link_list = "$home/inventory_delivery2.php?sorting_key=$sorting_key";
$link_upd = "$home/inventory_delivery2.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";

$link_list1 = "$home/inventory_delivery.php?sorting_key=$sorting_key";

$link_act = "$home/inventory_delivery2_act.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
?>
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='inventory_delivery22.php'>
			<div class='col-sm-2'>
...
$time_limit = 60*60*24*$notify_new_article; 
  ...
  if(!eregi("[^[:space:]]+",$key)) {
    $query = "SELECT uid,gate,gudang_code,shop_code,do_num,do_qty,date_do_post,date_do_check,date_do_done,date_do_sent,do_status 
      FROM shop_product_list_do WHERE $sorting_filter ORDER BY $sorting_key $sort_now";
  } else {
    $query = "SELECT uid,gate,gudang_code,shop_code,do_num,do_qty,date_do_post,date_do_check,date_do_done,date_do_sent,do_status 
      FROM shop_product_list_do WHERE $sorting_filter AND $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
  }
...
for($i = $first; $i <= $last; $i++) {
   ...
   $date_do_done = mysql_result($result,$i,8);
   $date_do_sent = mysql_result($result,$i,9);
   $do_status = mysql_result($result,$i,10);
	...
	if($date_do_check > 1) {
		if($lang == "ko") {
			$date_do_check_txt = "$date2Y"."/"."$date2m"."/"."$date2d";
		} else {
			$date_do_check_txt = "$date2d"."-"."$date2m"."-"."$date2Y";
		}
	} else {
			$date_do_check_txt = "<font color=red>x</font>";
	}
	
	$date3Y = substr($date_do_sent,0,4);
	$date3m = substr($date_do_sent,4,2);
	$date3d = substr($date_do_sent,6,2);
	
	if($date_do_sent > 1) {
		if($lang == "ko") {
			$date_do_sent_txt = "$date3Y"."/"."$date3m"."/"."$date3d";
		} else {
			$date_do_sent_txt = "$date3d"."-"."$date3m"."-"."$date3Y";
		}
	} else {
			$date_do_sent_txt = "<font color=red>x</font>";
	}
	
	$date4Y = substr($date_do_done,0,4);
	$date4m = substr($date_do_done,4,2);
	$date4d = substr($date_do_done,6,2);
	
	if($date_do_done > 1) {
		if($lang == "ko") {
			$date_do_done_txt = "$date4Y"."/"."$date4m"."/"."$date4d";
		} else {
			$date_do_done_txt = "$date4d"."-"."$date4m"."-"."$date4Y";
		}
	} else {
  ...
  echo("<td bgcolor='$highlight_color'>$date_do_sent_txt</td>");
            ...
            <ul role='menu' class='dropdown-menu' style='margin-left: -80px'>");
				// if($pay_status < 2) {
					if($date_do_check > 1) {
						...
						echo ("<li><a href='$link_act&act_mode=approve&uid=$do_uid'><i class='fa fa-check'></i> $txt_invn_stockout_30</a></li>");
					}
					if($date_do_sent > 1) {
						echo ("<li><a href='#'><font color=#AAAAAA><i class='fa fa-envelope'></i> $txt_invn_stockout_48 </a></font></li>");
					} else {
						echo ("<li><a href='$link_act&act_mode=send&uid=$do_uid'><i class='fa fa-envelope'></i> $txt_invn_stockout_49</a></li>");
					}
					echo ("<li class='divider'></li>");
				// }
				echo ("
                <li><a href='inventory_delivery_print.php?P_uid=$do_uid' target='_blank'><i class='fa fa-print'></i> $txt_invn_stockout_50</a></li>
            </ul>
		</div>
	...
	if($qs_sx_num != "") {
      ...
      echo ("
        <tr>
			...
			<td colspan=2>$qs_pcode {$dari_pname} {$dari_product_option1_txt}{$dari_product_option2_txt}</td>
