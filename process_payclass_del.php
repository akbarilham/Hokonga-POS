...
if(!strcmp($mode,"big")) {
	...
	$mid_query = "SELECT * FROM code_payclass2 WHERE lcode = '$bcode'";
	...
	if(!$mcode) {
		...
		$query = "DELETE FROM code_payclass1 WHERE lcode = '$bcode'";
	  ...
	  $query = "DELETE FROM code_payclass2 WHERE mcode = '$mcode'";
	  $result = mysql_query($query);
	  if(!$result) {
	    error("QUERY_ERROR");
	  exit;
