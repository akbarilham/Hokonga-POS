		        ...
		        $('.submit_on_enter_edit').keydown(function(event) {
		            if (event.keyCode == 13) {
		           		if (confirm("Yakin?"))
						{
				            	...
				            	if (!bind)
		           				bind = '0';
								var data = '?uid=' + id +'&val='+value+'&del=2&trans=<?=$trans?>&secuity=<?=$security?>' ;
			          	...
			          	<div class="row">
							<div class="col-lg-12 table-responsive" id='header_main' style='border: 0px solid #FFF;'>
										...
										<div class="task-progress" >
			                                  ...
			                                  <a id='tra'style='font-size:30px;'><?echo 'ES'.$kassier_aantal.'-'.substr($transaction_code,4)?>&nbsp;</a><a href="javascript:confirmVoidAll('<?=$void?>')" id='void' onclick="myFunction()" class="deletev fa fa-times-circle fa-2x" style='font-size:30px; width:20px; color:#FF0000;'>VOID</a>
			                            </div>

			                         		<div style='float:right;margin-left:-20px;width:250px;'>
											Total<p id='price' style='font-size:50px; width:200px;'><?=number_format($price)?></p>
											</div>
											<div style='float:right;padding:0 20px;width:120px;'>
											Quantity<p style='font-size:20px;' id='totqty' ><?=$qty?></p>
											</div>
											<div style='float:right;padding:0 20px;width:120px;'>
											Normal <p style='font-size:20px;' id='gross'><?=number_format($gross)?></p>
											</div>

											<div style='float:right;padding:0 20px;width:120px;'>
											Discount<p style='font-size:20px;' id='disc'>-<?=number_format($dic)?></p>
											</div>

											<div style='float:right;padding:0 20px;width:120px;'>
											Cashier<p style='font-size:20px;' id='cs'><?=$s_name[0]?></p>
											</div>
											...
											$dump = @mysql_result($result_pos,0,0);
												...
												$jmlCode = $arrDetail/$k;
														...
														echo "
															...
															<?$link = $home.'/pos_edit_void.php?trans='.$trans.'&uid=';?>
