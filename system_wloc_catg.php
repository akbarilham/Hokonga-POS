...
$mmenu = "system";
$smenu = "system_wloc_catg";

$link_list = "$home/system_wloc_catg.php";
$link_post = "$home/system_wloc_catg_post.php";
$link_upd = "$home/system_wloc_catg_upd.php";
$link_del = "$home/system_wloc_catg_del.php";
?>
...
echo ("
		...
		// Main Category
		$big_query = "SELECT * FROM wms_catgbig ORDER BY lcode";
		...
		while($big_row = mysql_fetch_object($big_result)) {
		  	...
		  	// 중분류
		  	$mid_query = "SELECT * FROM wms_catgmid WHERE lcode = '$big_code' ORDER BY mcode";
			...
			echo ("
			<form name='formB' method='post' action='$home/process_wloc_catg_updB.php'>
			...
			<td rowspan=$mid_rows>
				...
				<tr>
					...
					<td>
						...
						if($j2 == $big_rows) {
							echo("<a href='$home/process_wloc_catg_del.php?lang=$lang&mode=big&bcode=$big_code&bname=$big_name&gate=$gate'>$txt_comm_frm13</a>");
						} else {
			...
			");
		  	...
		  	while($mid_row = mysql_fetch_object($mid_result)) {
		    	...
		    	if($i == 0) {
		      	echo ("
				<form name='formM' method='post' action='$home/process_wloc_catg_updM.php'>
				<input type='hidden' name='lang' value='$lang'>
				<input type='hidden' name='gate' value='$login_gate'>
					...
					<tr>
						<td height=16>($m_code)
							...
							if($ss_mcode != $m_code) { echo ("
								<a href='system_wloc_catg.php?lang=$lang&ss_mode=add&ss_mcode=$m_code'><i class='fa fa-pencil'></i> Add</a>");
							} echo ("
						</td>
						...
						<td align=right>
							...
							if($i2 == $mid_rows) {
								echo("<a href='$home/process_wloc_catg_del.php?lang=$lang&mode=mid&mcode=$m_code&mname=$m_name&gate=$gate'>$txt_comm_frm13</a>");
							} else {
								echo("<font color=#AAAAAA>$txt_comm_frm13</font>");
							}
				...
				<td colspan=2>
					...
					// Small Category
					$sml_query = "SELECT * FROM wms_catgsml WHERE lcode = '$big_code' AND mcode = '$m_code' ORDER BY scode";
					...
					while($sml_row = mysql_fetch_object($sml_result)) {
						...
						echo ("
						<form name='formSU1' method='post' action='$home/process_wloc_catg_updS.php'>
						<input type='hidden' name='lang' value='$lang'>
						<input type='hidden' name='gate' value='$login_gate'>
						...
						<tr>
							...
							<td align=right>
								<input type='submit' value='$txt_comm_frm12' class='btn btn-default btn-xs'>
								<a href='$home/process_wloc_catg_delS.php?lang=$lang&scode=$s_code&gate=$login_gate'>$txt_comm_frm13</a>
							</td>
						</tr>
						</form>");
					}
					...
					echo ("
					<form name='formS1' method='post' action='$home/process_wloc_catg_addS.php'>
					<input type='hidden' name='lang' value='$lang'>
					<input type='hidden' name='gate' value='$login_gate'>
					<input type='hidden' name='m_catg' value='$m_code'>
					<tr>
				...
				<td colspan=2>
					...
					while($sml_row = mysql_fetch_object($sml_result)) {
						...
						echo ("
						<form name='formSU2' method='post' action='$home/process_wloc_catg_updS.php'>
						<input type='hidden' name='lang' value='$lang'>
						<input type='hidden' name='gate' value='$login_gate'>
						<input type='hidden' name='m_catg' value='$m_code'>
						<input type='hidden' name='new_scode' value='$s_code'>
					...
					if($ss_mode == "add" AND $ss_mcode == $m_code) {
					echo ("
					<form name='formS2' method='post' action='$home/process_wloc_catg_addS.php'>
					<input type='hidden' name='lang' value='$lang'>
					<input type='hidden' name='gate' value='$login_gate'>
					<input type='hidden' name='m_catg' value='$m_code'>
					<tr>
