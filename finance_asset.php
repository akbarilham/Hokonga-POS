...
$link_list = "$home/finance_asset.php?sorting_key=$sorting_key";
$link_upd = "$home/finance_asset.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
?>
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='finance_asset.php'>
			<div class='col-sm-2'>
...
?>
      ...
      echo ("
      <form name='signform2' class='cmxform form-horizontal adminex-form' method='post' action='finance_asset.php'>
	  ...
	  ?>
        ...
        <div class="panel-body">
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
											<select class="form-control" name="new_pt">
											<?
											if($login_level > '2') {
												echo("<option value=\"\">:: $txt_comm_frm24</option>");
											}
											$query_G1 = "SELECT count(uid) FROM client_branch";
											$result_G1 = mysql_query($query_G1,$dbconn);
												if (!$result_G1) { error("QUERY_ERROR"); exit; }
											$total_G1ss = @mysql_result($result_G1,0,0);
      
											$query_G2 = "SELECT uid,branch_code,branch_name FROM client_branch 
														 ORDER BY uid ASC";
											$result_G2 = mysql_query($query_G2,$dbconn);
											if (!$result_G2) { error("QUERY_ERROR"); exit; }   

											for($g = 0; $g < $total_G1ss; $g++) {
												$PT_uid = mysql_result($result_G2,$g,0);
												$PT_code = mysql_result($result_G2,$g,1);
												$PT_name = mysql_result($result_G2,$g,2);
												...
												echo("<option value='$PT_code' $PT_slct>[$PT_code] $PT_name</option>");
											}
											?>
											</select>
										</div>
									...
									<div class='form-group'>
                                        ...
                                        <div class='col-sm-3'>
											<input disabled type='text' class='form-control' name='new_p_code' value='$upd_p_code'>
										</div>
                                    </div>
									...
									<div class='form-group'>
                                        ...
                                        <div class='col-sm-3'>
											...
											if($upd_p_type == "2") {
												echo ("<option value='2' selected>Vehicle</option>");
											} else {
												echo ("<option value='2'>Vehicle</option>");
											}
											if($upd_p_type == "3") {
												echo ("<option value='3' selected>Inventory</option>");
											} else {
												echo ("<option value='3'>Inventory</option>");
											}
											if($upd_p_type == "4") {
												echo ("<option value='4' selected>Real Estate - Land</option>");
											} else {
												echo ("<option value='4'>Real Estate - Land</option>");
											}
											if($upd_p_type == "5") {
												echo ("<option value='5' selected>Real Estate - Building</option>");
											} else {
												echo ("<option value='5'>Real Estate - Building</option>");
											}
											if($upd_p_type == "6") {
												echo ("<option value='6' selected>Intellectual Property</option>");
											} else {
												echo ("<option value='6'>Intellectual Property</option>");
											}
											echo ("
											</select>
										</div>
        ...
        <div class="panel-body">
					...
					<div class="form-group ">
		                <label for="cname" class="control-label col-sm-3">Year</label>
		                <div class="col-sm-9">
							<input type="text" name="year" value='$year' class="form-control col-sm-4"/> 
						</div>
		            </div>
	  ...
	  <? } else if($mode == "request") { ?>
		...
		<?
		echo ("
		<form name='signform' class='cmxform form-horizontal adminex-form' method='post' action='finance_asset.php'>
        ...
        <div class="panel-body">
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-9">
											...
											for($g = 0; $g < $total_G1ss; $g++) {
												...
												echo("<option value='$PT_code'>[$PT_code] $PT_name</option>");
											}
											?>
											</select>
										</div>
									...
									<div class='form-group'>
                                        ...
                                        <div class='col-sm-3'>
											...
											<option value='4'>Real Estate - Land</option>
											<option value='5'>Real Estate - Building</option>
...
<?
  ...
  $new_p_detail = addslashes($new_p_detail);
    ...
    if($new_del_chk == "1") { // Delete
		...
		if($new_p_type == "2") { // Vehicle
		
			$query_D2 = "DELETE FROM asset_list_vehicle WHERE p_code = '$new_p_code'";
        ...
        $result_CHG1 = mysql_query("UPDATE asset SET gate = '$new_pt', shop_code = '$new_shop_code', 
            p_type = '$new_p_type', p_name = '$new_p_name', p_detail = '$new_p_detail', 
		...
		if($new_p_type == "2") { // Vehicle
			...
			$result_CHG2 = mysql_query("UPDATE asset_list_vehicle SET gate = '$new_pt', shop_code = '$new_shop_code', 
				p_name = '$new_p_name' WHERE p_code = '$new_p_code'",$dbconn);
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=$home/finance_asset.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page'>");
    exit;
  ...
  $m_ip = getenv('REMOTE_ADDR');
    ...
    $query_F1 = "INSERT INTO asset (uid,branch_code,gate,shop_code,p_code,p_type,p_name,p_detail,currency,
        qty,unit,unit_amount,total_amount,p_status,purchase_date,post_date) values ('',
        '$login_branch','$new_pt','$new_shop_code','$new_p_code','$new_p_type','$new_p_name','$new_p_detail','$new_currency',
	...
	if($new_p_type == "2") { // Vehicle
		...
		$query_F2 = "INSERT INTO asset_list_vehicle (uid,branch_code,gate,shop_code,p_code,p_name) values ('',
			'$login_branch','$new_pt','$new_shop_code','$new_p_code','$new_p_name')";
    ...
    echo("<meta http-equiv='Refresh' content='0; URL=$home/finance_asset.php?mode=check&sorting_key=$sorting_key&keyfield=$keyfield&key=$key&uid=$new_uid&page=$page'>");
    exit;
