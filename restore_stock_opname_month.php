...
$link_list = "$home/restore_stock_opname_month.php";
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(pnum) FROM table_stock_201507_cvj";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(pnum) FROM table_stock_201507_cvj WHERE $keyfield LIKE '%$key%'";  
}
...
$query_ini = "SELECT count(pnum) FROM table_stock_201507_cvj";
...
?>
        ...
        <div class="panel-body">
			...
			echo ("
			<form name='search' method='post' action='restore_stock_opname_month.php'>
			<div class='col-sm-3'>
        ...
        <?
		if(!eregi("[^[:space:]]+",$key)) {
			$query = "SELECT *,pnum,org_pcode FROM table_stock_201507_cvj ORDER BY $sorting_key $sort_now";
		} else {
			$query = "SELECT *,pnum,org_pcode FROM table_stock_201507_cvj WHERE $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
		}
				...
				<?
					...
					for($num=1; $num<=$total_ini_store; $num++) {
						echo ("<th colspan=6><div style='width: 100px'><a href='#' class='tooltips' title=\"\" data-toggle='tooltip' data-placement='top' 
							data-original-title='$br_branch_name &gt; $gd_shop_name'>TOKO $num</a></div></th>");
					...
					echo ("<tr>");
						for($num=1; $num<=$total_ini_store; $num++) {
							$num2 = sprintf("%02d", $num);
							 $query2 = "SELECT pnum, org_pcode,{$num2}_stock_awal,{$num2}_stock_in,{$num2}_stock_out,
                                {$num2}_stock_now,{$num2}_stock_check,{$num2}_stock_loss
                                FROM table_stock_201507_cvj";
		                        ...
		                        $stok_awal = mysql_result($result2,$i2,2);
		                        $stok_in = mysql_result($result2,$i2,3);
		                        $stok_out = mysql_result($result2,$i2,4);
		                        $stok_now = mysql_result($result2,$i2,5);
		                        $stok_check = mysql_result($result2,$i2,6);
		                        $stok_loss = mysql_result($result2,$i2,7);
