  ...
  <?if($void == 'void'){?>
	...
	<div class="row" style="border: 0px solid red; margin-left: 0%;">
		<div style="float:left; border: 0px solid green">
			<div> No : ES-<?=substr($transaction_code, 2)?> </div>
		</div>
	...
	// Show all carts
	$query_pay = "SELECT uid,detail,datedetail,temp,dump FROM pos_detail_backup
...
<div class="page-break">
	...
	<div class="row" style="border: 0px solid red; margin-left: 0%;">
		<div style="float:left; border: 0px solid green">
			<div> No : BS-<?=substr($transaction_code, 2)?> </div>
		</div>
		<div id="pageke" class="bok" style="float: left; margin-left: 20%"><?=$test?></div>
		<div style="float: left; border: 0px solid blue; margin-left: 22%">
			<div> <?=date("d/m/Y H:i", strtotime($trx_date))?> </div>
	...
	for ($i; $i <$totuid_K3; $i++) {
		...
		$pcode_2 = $detail_ex[0];
		$barcode_2 = $detail_ex[1];
		$price_2 = $detail_ex[3];
		$disc_rate_2 = $detail_ex[4];
		$gross_2 = $detail_ex[5];
		$nett_2 = $detail_ex[6];
		$vat_2 = $detail_ex[8];
		$qty_k_2 = $dump_ex_2[1];
