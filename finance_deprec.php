...
$link_list = "$home/finance_deprec.php?sorting_key=$sorting_key";
$link_upd = "$home/finance_deprec.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
?>
...
// 총 자료수
$queryAll = "SELECT count(uid) FROM asset_deprec WHERE $sorting_filter";
...
// Total Count ---------------------------------------------------- //
if(!eregi("[^[:space:]]+",$key)) {
  $query = "SELECT count(uid) FROM asset_deprec WHERE $sorting_filter GROUP BY p_code";
} else {
  $encoded_key = urlencode($key);
  $query = "SELECT count(uid) FROM asset_deprec WHERE $sorting_filter AND $keyfield LIKE '%$key%' GROUP BY p_code";  
}
...
// SUM
if(!eregi("[^[:space:]]+",$key)) {
  $query_sum_1 = "SELECT sum(accumulated_deprect) FROM asset_deprec 
...
if(!eregi("[^[:space:]]+",$key)) {
  $query_sum_2 = "SELECT sum(book_value) FROM asset_deprec 
...
$time_limit = 60*60*24*$notify_new_article; 
  ...
  if(!eregi("[^[:space:]]+",$key)) {
    $query = "SELECT uid,gate,p_code,p_type,p_name,currency,unit,year,deprec_percent,deprec_for,MAX(deprec_month),accumulated_deprect,book_value,purchase_date 
      FROM asset_deprec WHERE $sorting_filter GROUP BY p_code ORDER BY $sorting_key $sort_now ";
  } else {
    $query = "SELECT uid,gate,p_code,p_type,p_name,currency,unit,year,deprec_percent,deprec_for,MAX(deprec_month),accumulated_deprect,book_value,purchase_date 
      FROM asset_deprec WHERE $sorting_filter AND $keyfield LIKE '%$key%' GROUP BY p_code ORDER BY $sorting_key $sort_now";
  }
...
for($i = $first; $i <= $last; $i++) {
   ...
   $p_unit = mysql_result($result,$i,6);
   $p_year = mysql_result($result,$i,7);
   $p_deprec_percent = mysql_result($result,$i,8);
   $p_deprec_for = mysql_result($result,$i,9);
   	$p_deprec_for_K3 = number_format($p_deprec_for);
   $p_deprec_month = mysql_result($result,$i,10);
   	$p_deprec_month_K3 = date('M Y', strtotime($p_deprec_month));
   $p_accumulated_deprect = mysql_result($result,$i,11);
   	$p_accumulated_deprect_K3 = number_format($p_accumulated_deprect);
   $p_book_value = mysql_result($result,$i,12);
   	$p_book_value_K3 = number_format($p_book_value);
   $purchase_date = mysql_result($result,$i,13);
   
   if($p_currency == $now_currency1) {
		$p_total_amount_1 = $p_deprec_for;
		$p_total_amount_2 = "0";
		
   } else if($p_currency == $now_currency2) {
		$p_total_amount_1 = "0";
		$p_total_amount_2 = $p_deprec_for;
   }
   ...
   $p_total_amount_2_k = number_format($p_total_amount_2); // Dicimal Point
	...
	// Nama PT
	$query_PT = "SELECT branch_name2 FROM client_branch WHERE branch_code = '$p_gate'";
	$result_PT = mysql_query($query_PT);
	$p_PT = mysql_result($result_PT,0,0);
  ...
  echo ("<tr>");
	  ...
	  echo("<td bgcolor='$highlight_color'><font color='$font_color'>$p_PT</font></td>");
	  echo("<td bgcolor='$highlight_color'><a href='$link_upd&mode=check&pcode=$p_code'><font color='$font_color'>($p_code) $p_name</font></a></td>");
	  ...
	  echo("<td bgcolor='$highlight_color'><font color='$font_color'>$p_deprec_percent %</font></td>");
	  echo("<td bgcolor='$highlight_color'><a href='#'><span class='badge bg-info'>$p_deprec_month_K3</span></a></td>");
	  echo("<td>$p_accumulated_deprect_K3</td>");
	  echo("<td>$p_book_value_K3</td>");
  echo("</tr>");
  
  if($mode == "check" AND $pcode == $p_code) {

    $query_totale_D1 = "SELECT count(p_code) FROM asset_deprec WHERE p_code = '$p_code'";
    $fetch_totale_D1 = mysql_query($query_totale_D1);
    if (!$fetch_totale_D1) { error("QUERY_ERROR");   }
	$totale_D1 = mysql_result($fetch_totale_D1,0,0);

	  for ($k = 0;$k<$totale_D1;$k++) {

	    $query_D1 = "SELECT uid,gate,p_code,p_name,deprec_month,accumulated_deprect,book_value
	      FROM asset_deprec WHERE p_code = '$p_code'";
	    $result_D1 = mysql_query($query_D1);
		if (!$result_D1) {   error("QUERY_ERROR");   exit; }	  

		   $D_uid = mysql_result($result_D1,$k,0);
		   $D_gate = mysql_result($result_D1,$k,1);
		   $D_code = mysql_result($result_D1,$k,2);
		   $D_name = mysql_result($result_D1,$k,3);
		   $D_deprec_month = mysql_result($result_D1,$k,4);
		   $D_accumulated_deprect = mysql_result($result_D1,$k,5);
		   $D_book_value = mysql_result($result_D1,$k,6);

		   // Nama PT
		   $query_PT2 = "SELECT branch_name2 FROM client_branch WHERE branch_code = '$D_gate'";
		   $result_PT2 = mysql_query($query_PT2);
		   $D_PT = mysql_result($result_PT,0,0);

		   // Filtering
		   $D_name = stripslashes($D_name);
		   $D_accumulated_deprect_K3 = number_format($D_accumulated_deprect);
		   $D_book_value_K3 = number_format($D_book_value);
		   $D_deprec_month_K3 = date('M Y', strtotime($D_deprec_month));

			echo ("
			<tr>
				<td></td>
				<td>$D_PT</td>
				<td colspan=6><a href='$link_upd'>
					($D_code) $D_name
				</a></td>
				<td><span class='badge bg-info'>$D_deprec_month_K3</span></td>
				<td>$D_accumulated_deprect_K3</td>
				<td>$D_book_value_K3</td>		
			</tr>");
		}
  }  
...
?>
				...
				<a href="<?echo("$link_upd&mode=request")?>"><input type="button" value="New Depreciation" class="btn btn-primary"></a>
		...
		/*if($mode == "check" AND $uid) { // Update
          ...
          $pm_query = "SELECT uid,gate,p_code,p_type,p_name,currency,unit,year,deprec_percent,
          deprec_for,deprec_month,accumulated_deprect,book_value,purchase_date FROM asset_deprec WHERE uid = '$uid'";
...
<?
  ...
  $new_p_detail = addslashes($new_p_detail);
    ...
    if($new_del_chk == "1") { // Delete
        ...
        $query_D1 = "DELETE FROM asset_deprec WHERE uid = '$new_p_uid'";
        $result_D1 = mysql_query($query_D1);
        if (!$result_D1) { error("QUERY_ERROR"); exit; }
