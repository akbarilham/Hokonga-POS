...
$link_list = "$home/sales_pr_order.php";
...
if(!$sorting_key) { $sorting_key = "order_date"; }
...
// 장바구니에 담긴 상품 수
$query_HC = "SELECT count(uid) FROM shop_cart WHERE user_id = '$login_id' AND f_class = 'b2b' AND expire = '0'";
  ...
  <section id="container" class="">
      ...
      <section id="main-content">
		  ...
		  <? include "navbar_inventory_pr.inc"; ?>
			...
			<?
			echo ("
			<form name='search' method='post' action='sales_pr_order.php'>
			<div class='col-sm-2'>
...
for($i = $first; $i <= $last; $i++) {
  ...
  // Print
  echo("<td><a class='btn btn-default btn-xs' target='_blank' href='sales_pr_order_print.php?P_uid=$uid'><i class='fa fa-print'></i> Print </a></td>");
  ...
  echo("</tr>");
			...
			$query_HCc = "SELECT count(uid) FROM shop_cart WHERE order_num = '$now_po_num' AND f_class = 'b2b' AND expire = '1'";
			...
			$total_HCc = @mysql_result($result_HCc,0,0);
				...
				$query_H = "SELECT uid,prd_uid,pcode,qty,product_color,product_size,p_option1,p_option2,p_option3,p_option4,p_option5,
							...
							WHERE order_num = '$now_po_num' AND f_class = 'b2b' AND expire = '1' ORDER BY date ASC";
				...
				for($h = 0; $h < $total_HCc; $h++) {
					...
					<tr>
						<td></td>
						<td colspan=2>
							[$H_org_pcode] $dari_pname {$H_p_opt1_txt}{$H_p_opt2_txt}{$H_p_opt3_txt}{$H_p_opt4_txt}{$H_p_opt5_txt}
						</td>
...
?>
				...
				<a href="sales_pr_order.php?mode=show_cart"><button class="btn btn-primary"><i class="fa fa-shopping-cart"></i> <?=$txt_invn_purchase_13pr?></button></a>
	  ...
	  <?
	  $query_H = "SELECT uid,prd_uid,pcode,qty,product_color,product_size,p_option1,p_option2,p_option3,p_option4,p_option5,
				p_name,p_price,unpack_qty,unpack_unit_uid,unpack_unit_qty,unpack_unit_name,currency,org_pcode,org_barcode FROM shop_cart 
				WHERE user_id = '$login_id' AND f_class = 'b2b' AND expire = '0' ORDER BY date ASC";
      $result_H = mysql_query($query_H);
      if (!$result_H) {   error("QUERY_ERROR");   exit; }
      ...
      echo ("
      <form name='cart_upd' method='post' action='sales_pr_order_cart.php'>
      ...
      <tr height=22>
        ...
        <form name='cart_del' method='post' action='sales_pr_order_cart.php'>
      ...
      // 합계
      echo ("
      <form name='order_check' method='post' action='sales_pr_order.php'>
      ...
      <tr height=22>
		<td colspan=2 align=center>");
			...
			if($now_group_admin == "1") {
                  ...
                  echo("<select name='key_branch' class='form-control' required>");
	  ...
	  */
      ...
      echo ("
      <form name='signform' class='cmxform form-horizontal adminex-form' method='post' action='sales_pr_order_cart.php'>
      <input type=hidden name='mode' value='order_form'>
	  ...
	  if($now_group_admin == "1") {
		...
		<div class='form-group'>
			<label class='control-label col-sm-2'>$txt_invn_stockout_05</label>
            <div class='col-sm-4'>");
			
				  // Shop
                  $query_E1C = "SELECT count(uid) FROM client_shop WHERE branch_code = '$key_branch' AND userlevel > '0'";
                  $result_E1C = mysql_query($query_E1C);
                  if (!$result_E1C) { error("QUERY_ERROR"); exit; }
    
                  $total_E1C = @mysql_result($result_E1C,0,0);

                  $query_E1 = "SELECT uid,shop_code,shop_name,associate FROM client_shop WHERE branch_code = '$key_branch' AND userlevel > '0' ORDER BY shop_code ASC";
                  $result_E1 = mysql_query($query_E1);
                  if (!$result_E1) { error("QUERY_ERROR"); exit; }
                  
                  echo("<select name='new_shop_code' class='form-control' required>");
				  echo("<option value=\"\">:: $txt_comm_frm241</option>");
				  
                  for($e1 = 0; $e1 < $total_E1C; $e1++) {
                    $E1_uid = mysql_result($result_E1,$e1,0);
                    $E1_shop_code = mysql_result($result_E1,$e1,1);
                    $E1_shop_name = mysql_result($result_E1,$e1,2);
					$E1_shop_assoc = mysql_result($result_E1,$e1,3);
					
					if($E1_shop_code == $key_shop) {
						$E1_code_slct = "selected";
					} else {
						$E1_code_slct = "";
					}
                    
                    echo ("<option value='$E1_shop_code' $E1_code_slct>[$E1_shop_code] $E1_shop_name</option>");
                  }
                  echo ("</select>");
				  echo ("
		...
		<div class='form-group'>
            ...
            <div class='col-sm-4'>");
				  ...
				  // Shop
                  $query_E1C = "SELECT count(uid) FROM client_shop WHERE branch_code = '$login_branch' AND userlevel > '0'";
                  $result_E1C = mysql_query($query_E1C);
                  if (!$result_E1C) { error("QUERY_ERROR"); exit; }
    
                  $total_E1C = @mysql_result($result_E1C,0,0);

                  $query_E1 = "SELECT uid,shop_code,shop_name,associate FROM client_shop WHERE branch_code = '$login_branch' AND userlevel > '0' ORDER BY shop_code ASC";
                  $result_E1 = mysql_query($query_E1);
                  if (!$result_E1) { error("QUERY_ERROR"); exit; }
  ...
  if($add_mode == "LIST_CHG") {
    ...
    if($new_pay_status == "4") { // 결제처리 - 은행대출 신청[인보이스 발행], 실제 대출 신청액 산정
        ...
        $query_C2 = "INSERT INTO shop_cart (uid,prd_uid,branch_code,gate,f_class,user_id,user_ip,pcode,qty,
            ...
            values ('','$new_prd_uid','$br_branch_code','$new_gudang_code','b2b','$login_id','$m_ip',
    ...
    // 리스트로 되돌아 가기
    echo("<meta http-equiv='Refresh' content='0; URL=$home/sales_pr_order.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page'>");
    exit;
      ...
      for($r = 0; $r < $total_RC; $r++) {
        ...
        $r_total_price_orgin = $r_total_price_orgin + ($r_price_orgin * $r_stock_org);
          ...
          $query_C2 = "INSERT INTO shop_cart (uid,prd_uid,branch_code,gate,f_class,user_id,user_ip,pcode,qty,
            ...
            '$r_branch_code','$r_gudang_code','b2b','$login_id','$m_ip','$r_code','$r_stock_org',
