...
?>
        ...
        <header class="panel-heading">
            <?=$hsm_name_08_032?> &gt; <?=$txt_web_banner2_03?>
        ...
        <div class="panel-body">
							...
							<div class="form">
                                <form class="cmxform form-horizontal adminex-form" name="signformnow" method="post" action="website_banner2_upd.php">
		...
		echo ("
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-3">
											...
											<?
											$query_user1 = "SELECT count(code) FROM member_main 
												WHERE gate = '$key_gate' AND branch_code = '$login_branch' AND mb_type = '4'";
											$result_user1 = mysql_query($query_user1);
											$total_user = @mysql_result($result_user1,0,0);

											$query_user = "SELECT code,name FROM member_main 
												WHERE gate = '$key_gate' AND branch_code = '$login_branch' AND mb_type = '4' ORDER BY name ASC, code ASC";
											$result_user = mysql_query($query_user);
			
											for($u = 0; $u < $total_user; $u++) {
												$in_user_code = mysql_result($result_user,$u,0);
												$in_user_name = mysql_result($result_user,$u,1);
			
												if($user_code == $in_user_code) {
													echo ("<option value='$in_user_code' selected> [ $in_user_code ] $in_user_name</option>");
												} else {
													echo ("<option value='$in_user_code'> [ $in_user_code ] $in_user_name</option>");
												}
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control" name="b_alt" value="<?=$b_alt?>">
                                        </div>
										<div class="col-sm-3"><?=$txt_web_banner2_09?></div>
										<div class="col-sm-3">
                                            <input type="color" class="form-control" name="b_bgcolor" value="<?=$b_bgcolor?>">
                                        </div>
                                    </div>
									...
									<? if($b_loco == "popup") { ?>
									<div class="form-group ">
                                        <label for="cname" class="control-label col-sm-3">Banner Location</label>
                                        <div class="col-sm-3">
											<? echo ("
                                            Left : <input type='text' name='b_left' value='$b_left' maxlength=4 style='$style_box; WIDTH: 50px; text-align:center;'> px (195) &nbsp;&nbsp;&nbsp;&nbsp; 
											Top : <input type='text' name='b_top' value='$b_top' maxlength=4 style='$style_box; WIDTH: 50px; text-align:center;'> px (317)");
											?>
                                        </div>
										<div class="col-sm-3">Font Color</div>
										<div class="col-sm-3">
                                            <input type="color" class="form-control" name="b_fontcolor" value="<?=$b_fontcolor?>">
                                        </div>
                                    </div>
									<? } ?>
									...
									<div class="form-group ">
                                        ...
                                        <div class="col-sm-3">
												...
												<? echo ("
												<option value='_top' $selectA>_top</option>
												<option value='_blank' $selectB>_blank</option>
												<option value='_self' $selectC>_self</option>
												<option value='_new' $selectD>_new</option>
												<option value='1' $selectE>Custom</option>");
												?>
												</select>
                                        </div>
										...
										<div class="col-sm-3">
												<?
												if($input == "1") {
													echo ("<input type=text name='b_targetB' value='$b_target' maxlength=20 class='form-control'>");
												} else {
													echo ("<input type=text name='b_targetB' maxlength=20 class='form-control'>");
												}
												?>
                                        </div>
                                    </div>
...
<?
	...
	if($b_loco == "popup") {
		$query_PO = "UPDATE wpage_banner2 SET fontcolor = '$b_fontcolor', b_left = '$b_left', b_top = '$b_top' 
					WHERE uid = '$uid' AND gate = '$key_gate' AND branch_code = '$login_branch'";
		$result_PO = mysql_query($query_PO);
		if (!$result_PO) { error("QUERY_ERROR"); exit; }
	}
	
	
	
	if($b_loco == "main_mid") {
		$query_V = "UPDATE wpage_banner2 SET videoclip_play = '$videoclip_play', videoclip = '$videoclip' 
					WHERE uid = '$uid' AND gate = '$key_gate' AND branch_code = '$login_branch'";
	...
	$query_M1 = "UPDATE wpage_banner2 SET b_ord = '$b_ord', user_code = '$user_code', bgcolor = '$b_bgcolor', 
         b_alt = '$b_alt', b_url = '$b_url', b_target = '$b_target', b_show = '$b_show', upd_date = '$dates' 
		 WHERE uid = '$uid' AND gate = '$key_gate' AND branch_code = '$login_branch'";
