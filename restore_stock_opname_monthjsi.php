...
$mmenu = "restore";
$smenu = "restore_stock_opname_monthjsi";
...
$link_list = "$home/restore_stock_opname_monthjsi.php";
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT count(org_pcode) FROM table_stock_201507_jsi";
} else {
   $encoded_key = urlencode($key);
   $query = "SELECT count(org_pcode) FROM table_stock_201507_jsi WHERE $keyfield LIKE '%$key%'";  
}
...
$query_ini = "SELECT count(org_pcode) FROM table_stock_201507_jsi";
...
?>
        ...
        <div class="panel-body">
			...
			echo ("
			<form name='search' method='post' action='restore_stock_opname_monthjsi.php'>
			<div class='col-sm-3'>
        ...
        <?
		if(!eregi("[^[:space:]]+",$key)) {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_jsi ORDER BY $sorting_key $sort_now";
		} else {
			$query = "SELECT pnum,org_pcode,stocks FROM table_stock_201507_jsi WHERE $keyfield LIKE '%$key%' ORDER BY $sorting_key $sort_now";
		}
		...
		<div class='col-sm-9'>
				...
				<tr>
				<?
					$store_branch = "CORP_02";
					$store_branch_name = "JSI";
					$total_ini_store = 138;
					...
					for($num=1; $num<=$total_ini_store; $num++) {
						...
						$query_str = "SELECT shop_code FROM table_store_name_jsi WHERE num = '$num2'";
...
<?
	...
	echo("<meta http-equiv='Refresh' content='0; URL=$home/restore_stock_opname_monthjsi.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
	...
	echo("<meta http-equiv='Refresh' content='10; URL=$home/restore_stock_opname_monthjsi.php?sorting_key=$sorting_key&sort_now=$sort_now&keyfield=$keyfield&key=$key&page=$page'>");
	exit;
