...
$link_list = "$home/inventory_purchase1.php?sorting_key=$sorting_key";
$link_upd = "$home/inventory_purchase1.php?sorting_key=$sorting_key&keyfield=$keyfield&key=$key&page=$page";
...
?>
        ...
        <div class="panel-body">
			...
			<?
			echo ("
			<form name='search' method='post' action='inventory_purchase1.php'>
			<div class='col-sm-2'>
...
if(!eregi("[^[:space:]]+",$key)) {
   $query = "SELECT uid,branch_code,client_code,manager_code,po_num,po_qty,po_tamount,products,order_date,currency,order_status,do_status,pay_status,check_status,xchg_rate,unit,eta_date,eta_status,revisi,wrong_status,po_ppn
...
for($i = $first; $i <= $last; $i++) {
   ...
   // 3: 입고 완료
	...
	// Customer Info
	$mb_query = "SELECT uid,supp_name,supp_code,email,phone1,phone_fax,zipcode,addr1,addr2 FROM client_supplier WHERE supp_code = '$client_code'";
  ...
  echo("<td bgcolor='$highlight_color'><a href='$link_list&mode=view&now_po_num=$po_num&uid=$li_uid&page=$page'>$po_num</a> $date_diff_warning</td>");
  ...
  echo("<td bgcolor='$highlight_color'>
		...
		<div class='btn-group'>
            ...
            <ul role='menu' class='dropdown-menu' style='margin-left: -120px'>");
				// if($pay_status < 2) {
					if($order_status > 1) {
					...
					echo ("
					<li><a href='$link_act&act_mode=send&uid=$li_uid'><i class='fa fa-envelope'></i> $txt_invn_stockin_49 to:<br>$mb_corp_name<br><i>$mb_email</i></a></li>
					<li class='divider'></li>");
				// }
				echo ("
